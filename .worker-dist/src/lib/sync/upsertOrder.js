...
async function upsertNormalizedOrder(n) {
    ...
    const totalAmount = items.reduce((s, it) => s + ((0, parseNumber_1.parseNumber)(it.salePrice || 0) * (0, parseNumber_1.parseIntLike)(it.qty || 0)), 0);
    ...
    if (existing) {
        ...
        for (const it of items) {
            ...
            if (!prod) {
                prod = (await p.product.create({ data: { sku: it.externalSku, name: it.title || it.externalSku, category: "unknown", sellingPrice: (0, parseNumber_1.parseNumber)(it.salePrice || 0) } }));
            }
            const oi = await p.orderItem.create({ data: { orderId: existing.id, productId: prod.id, quantity: Math.max(1, (0, parseNumber_1.parseIntLike)(it.qty || 0)), sellingPrice: (0, parseNumber_1.parseNumber)(it.salePrice || 0) } });
    ...
    for (const it of items) {
        ...
        const oi = await p.orderItem.create({ data: { orderId: created.id, productId: prod.id, quantity: Math.max(1, (0, parseNumber_1.parseIntLike)(it.qty || 0)), sellingPrice: (0, parseNumber_1.parseNumber)(it.salePrice || 0) } });
        createdItems.push(oi);
    }
