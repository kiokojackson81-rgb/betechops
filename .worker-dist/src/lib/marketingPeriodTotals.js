...
async function summarizeMarketingReportsForPeriod(opts) {
    ...
    const perReceipts = new Map();
    const recKey = (s) => (s ?? "").trim();
    const markIfNew = (key) => {
    ...
    marketingEntries.forEach((entry) => {
        ...
        if (sales.length > 0) {
            ...
            sales.forEach((sale, index) => {
                const method = normalizeMethod(sale.paymentMethod);
                const receiptIdBase = recKey(sale.receiptNumber) || `${entry.id}-${index}`;
                if (entrySeen.has(receiptIdBase))
                    return;
    ...
    reports.forEach((report) => {
        ...
        sales.forEach((sale, index) => {
            const method = normalizeMethod(sale.paymentMethod);
            const receiptIdBase = sale.receiptNumber && sale.receiptNumber.trim().length > 0
                ? sale.receiptNumber.trim()
                : `${report.id}-${index}`;
            if (entrySalesReceiptKeys.has(receiptIdBase))
                return;
...
async function recomputeMarketingCommissionLedger(opts) {
    ...
    if (totalSales <= 0) {
        ...
        const profitPortion = totalProfit > 0 ? Math.min((500000 / totalSales) * totalProfit, totalProfit) : 0;
        const baseCommission = Math.round(profitPortion * 0.05);
        // Now compute completed tier rewards + prorated reward for current band.
        let additional = 0;
        const ladder = [...commissionCommon_1.COMMISSION_LADDER].sort((a, b) => a.min - b.min);
        if (ladder.length === 0) {
            ...
            if (totalSales <= firstMin) {
                const bandSize = firstMin - 500000;
                const progress = Math.max(0, Math.min(totalSales - 500000, bandSize));
