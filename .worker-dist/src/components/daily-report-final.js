...
const daily_report_receipts_1 = __importDefault(require("./daily-report-receipts"));
...
const cardClasses = "rounded-3xl border border-slate-800 bg-slate-800/70 shadow-2xl shadow-black/40";
...
function DailyReportFinal() {
    const [showMyReceipts, setShowMyReceipts] = (0, react_2.useState)(false);
    // receipts-history controls (used when #my-receipts)
    const todayIso = new Date().toISOString().split("T")[0];
    const [startDate, setStartDate] = (0, react_2.useState)(todayIso);
    const [endDate, setEndDate] = (0, react_2.useState)(todayIso);
    const [searchTerm, setSearchTerm] = (0, react_2.useState)("");
    const [debouncedSearch, setDebouncedSearch] = (0, react_2.useState)("");
    const [receiptsSummary, setReceiptsSummary] = (0, react_2.useState)({ count: 0, totalSales: 0 });
    (0, react_2.useEffect)(() => {
    ...
    const sessionResponse = (0, react_1.useSession)();
    const session = sessionResponse?.data;
    const attendantId = impersonateId ?? (session?.user?.id ?? null);
    ...
    const [selectedPeriod, setSelectedPeriod] = (0, react_2.useState)(currentPeriod);
    ...
    const fetchPeriodSummary = (0, react_2.useCallback)(async (signal) => {
        if (!selectedPeriodKey || typeof window === "undefined")
            return null;
        try {
    ...
    const datePicker = ((0, jsx_runtime_1.jsxs)("div", { className: "flex items-center gap-2", children: [(0, jsx_runtime_1.jsx)(lucide_react_1.CalendarIcon, { size: 16, className: "text-slate-400" }), (0, jsx_runtime_1.jsx)("input", { type: "date", value: date, onChange: (e) => {
                    ...
                    if (!Number.isNaN(d.getTime())) {
                        setDayOfWeek(d.toLocaleDateString(kenyanLocale, { weekday: "long", timeZone: kenyaTimeZone }));
                    }
                }, className: inputClasses }), (0, jsx_runtime_1.jsxs)("div", { className: "flex gap-2", children: [(0, jsx_runtime_1.jsx)("button", { type: "button", onClick: () => {
                            const d = new Date();
                            const iso = d.toISOString().split("T")[0];
                            setDate(iso);
                            setDayOfWeek(d.toLocaleDateString(kenyanLocale, { weekday: "long", timeZone: kenyaTimeZone }));
                        }, className: "rounded-full border border-white/10 bg-slate-900/60 px-3 py-1 text-xs font-semibold text-slate-200 hover:bg-slate-900/70", children: "Today" }), (0, jsx_runtime_1.jsx)("button", { type: "button", onClick: () => {
                            const d = new Date();
                            d.setDate(d.getDate() - 1);
                            const iso = d.toISOString().split("T")[0];
                            setDate(iso);
                            setDayOfWeek(d.toLocaleDateString(kenyanLocale, { weekday: "long", timeZone: kenyaTimeZone }));
                        }, className: "rounded-full border border-white/10 bg-slate-900/60 px-3 py-1 text-xs font-semibold text-slate-200 hover:bg-slate-900/70", children: "Yesterday" })] })] }));
    ...
    if (showMyReceipts) {
        return ((0, jsx_runtime_1.jsx)("div", { className: "min-h-screen bg-slate-950 text-slate-100", children: (0, jsx_runtime_1.jsxs)("main", { className: "mx-auto max-w-5xl space-y-6 p-6", children: [(0, jsx_runtime_1.jsxs)("header", { className: "flex items-start justify-between gap-4", children: [(0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("h1", { className: "text-3xl font-semibold", children: "Receipts history" }), (0, jsx_runtime_1.jsx)("p", { className: "text-sm text-slate-300", children: "Browse every receipt captured in the system. Use the range pills or custom dates to narrow the window." })] }), (0, jsx_runtime_1.jsx)(link_1.default, { href: "/attendant/daily-report", className: "rounded-full border border-white/20 bg-white/5 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-100 transition hover:border-white/40 hover:bg-white/10", children: "Back to dashboard" })] }), (0, jsx_runtime_1.jsxs)(Card_1.default, { className: "space-y-5 border-slate-800 bg-slate-900/80 shadow-xl shadow-black/40", children: [(0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between", children: [(0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("p", { className: "text-xs uppercase tracking-[0.2em] text-slate-400", children: "Receipts list" }), (0, jsx_runtime_1.jsx)("h2", { className: "text-lg font-semibold text-slate-100", children: "Read-only receipts history" }), (0, jsx_runtime_1.jsx)("p", { className: "text-sm text-slate-400", children: "Explore every receipt captured across the system and filter by date, range, or attendant." })] }), (0, jsx_runtime_1.jsxs)("div", { className: "flex flex-wrap gap-2 text-[11px] font-semibold uppercase tracking-wide", children: [(0, jsx_runtime_1.jsx)("button", { onClick: () => setRange("today"), className: `rounded-full border px-4 py-1 transition ${startDate === new Date().toISOString().split("T")[0] && endDate === new Date().toISOString().split("T")[0] ? "border-emerald-500 bg-emerald-500/20 text-emerald-200" : "border-white/15 text-slate-200 hover:border-emerald-500 hover:text-white"}`, children: "Today" }), (0, jsx_runtime_1.jsx)("button", { onClick: () => setRange("yesterday"), className: `rounded-full border px-4 py-1 transition ${(() => { const d = new Date(); d.setDate(d.getDate() - 1); const iso = d.toISOString().split("T")[0]; return startDate === iso && endDate === iso ? "border-emerald-500 bg-emerald-500/20 text-emerald-200" : "border-white/15 text-slate-200 hover:border-emerald-500 hover:text-white"; })()}`, children: "Yesterday" }), (0, jsx_runtime_1.jsx)("button", { onClick: () => setRange("thisWeek"), className: `rounded-full border px-4 py-1 transition ${(() => { const now = new Date(); const day = now.getDay(); const diffToMonday = day === 0 ? -6 : 1 - day; const monday = new Date(now); monday.setDate(now.getDate() + diffToMonday); const isoMonday = monday.toISOString().split("T")[0]; const isoToday = new Date().toISOString().split("T")[0]; return startDate === isoMonday && endDate === isoToday ? "border-emerald-500 bg-emerald-500/20 text-emerald-200" : "border-white/15 text-slate-200 hover:border-emerald-500 hover:text-white"; })()}`, children: "This week" }), (0, jsx_runtime_1.jsx)("button", { onClick: () => setRange("period"), className: "rounded-full border px-4 py-1 transition border-white/15 text-slate-200 hover:border-emerald-500 hover:text-white", children: "This period" })] })] }), (0, jsx_runtime_1.jsxs)("div", { className: "grid gap-3 lg:grid-cols-4", children: [(0, jsx_runtime_1.jsxs)("label", { className: "text-xs uppercase tracking-wide text-slate-400", children: ["Search", (0, jsx_runtime_1.jsx)("input", { placeholder: "Customer, attendant, receipt...", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), className: "mt-1 w-full rounded-xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-sm text-slate-100 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" })] }), (0, jsx_runtime_1.jsxs)("label", { className: "text-xs uppercase tracking-wide text-slate-400", children: ["Start date", (0, jsx_runtime_1.jsx)("input", { type: "date", value: startDate, onChange: (e) => setStartDate(e.target.value), className: "mt-1 w-full rounded-xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-sm text-slate-100 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" })] }), (0, jsx_runtime_1.jsxs)("label", { className: "text-xs uppercase tracking-wide text-slate-400", children: ["End date", (0, jsx_runtime_1.jsx)("input", { type: "date", value: endDate, onChange: (e) => setEndDate(e.target.value), className: "mt-1 w-full rounded-xl border border-slate-800 bg-slate-950/70 px-3 py-2 text-sm text-slate-100 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" })] })] }), (0, jsx_runtime_1.jsxs)("div", { className: "grid gap-3 sm:grid-cols-3", children: [(0, jsx_runtime_1.jsxs)("div", { className: "rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-3", children: [(0, jsx_runtime_1.jsx)("p", { className: "text-[11px] uppercase tracking-wide text-slate-400", children: "Range" }), (0, jsx_runtime_1.jsx)("p", { className: "text-sm font-semibold text-slate-100", children: "Selected" }), (0, jsx_runtime_1.jsxs)("p", { className: "text-xs text-slate-400", children: ["Showing receipts from ", startDate, " to ", endDate] })] }), (0, jsx_runtime_1.jsxs)("div", { className: "rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-3", children: [(0, jsx_runtime_1.jsx)("p", { className: "text-[11px] uppercase tracking-wide text-slate-400", children: "Receipts" }), (0, jsx_runtime_1.jsx)("p", { className: "text-2xl font-semibold text-emerald-300", children: receiptsSummary.count }), (0, jsx_runtime_1.jsx)("p", { className: "text-xs text-slate-400", children: "Captured in the selected window" })] }), (0, jsx_runtime_1.jsxs)("div", { className: "rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-3", children: [(0, jsx_runtime_1.jsx)("p", { className: "text-[11px] uppercase tracking-wide text-slate-400", children: "Total sales" }), (0, jsx_runtime_1.jsx)("p", { className: "text-2xl font-semibold text-emerald-300", children: formatKES(receiptsSummary.totalSales ?? 0) }), (0, jsx_runtime_1.jsx)("p", { className: "text-xs text-slate-400", children: "Aggregated from the receipts below" })] })] }), (0, jsx_runtime_1.jsx)("div", { children: (0, jsx_runtime_1.jsx)(daily_report_receipts_1.default, { start: startDate, end: endDate, q: debouncedSearch, attendantId: undefined, hideHeader: true, onSummary: (s) => setReceiptsSummary({ count: s.count, totalSales: s.totalSales }) }) })] })] }) }));
    }
    return ((0, jsx_runtime_1.jsxs)("div", { className: "min-h-screen bg-slate-950 text-slate-50 px-6 py-8 space-y-6", children: [(0, jsx_runtime_1.jsxs)("section", { className: "mb-2 space-y-6", children: [(0, jsx_runtime_1.jsx)("div", { className: "rounded-3xl border border-slate-800 bg-slate-950/70 px-6 py-4 md:px-8 md:py-5", children: (0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between", children: [(0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("h1", { className: "text-2xl lg:text-3xl font-semibold", children: "Marketing Operations Dashboard" }), (0, jsx_runtime_1.jsx)("p", { className: "text-slate-400 text-sm", children: "Daily tracker for uploads, engagement, walk-ins and live sessions." })] }), (0, jsx_runtime_1.jsx)("div", { children: (0, jsx_runtime_1.jsx)(HeaderActions_1.default, { receiptsHref: "#my-receipts", createHref: `/receipts?view=create`, onSignOut: () => (0, react_1.signOut)({ callbackUrl: "/attendant/login" }), onReceiptsClick: () => setShowMyReceipts(true), showDot: true }) })] }) }), (0, jsx_runtime_1.jsx)("div", { className: "flex flex-col gap-3 rounded-3xl border border-slate-800 bg-slate-950/70 px-6 py-4 md:px-8 md:py-5", children: (0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col gap-2 md:flex-row md:items-center md:justify-between", children: [(0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("p", { className: "text-xs uppercase tracking-[0.2em] text-slate-400", children: "Statistics period" }), (0, jsx_runtime_1.jsx)("p", { className: "text-lg font-semibold text-slate-50", children: selectedPeriodLabel }), selectedPeriodKey !== currentPeriod.key && ((0, jsx_runtime_1.jsx)("p", { className: "text-xs text-amber-300", children: "Showing archived period." }))] }), (0, jsx_runtime_1.jsx)(PeriodSwitcher_1.default, { currentPeriod: currentPeriod, selectedPeriod: selectedPeriod, onSelectPeriod: setSelectedPeriod })] }) }), (0, jsx_runtime_1.jsx)("section", { className: "rounded-3xl border border-slate-800 bg-slate-950/70 px-6 py-4 md:px-8 md:py-5", children: (0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col gap-4 md:flex-row md:items-center md:gap-8", children: [(0, jsx_runtime_1.jsx)("div", { className: "flex-1", children: (0, jsx_runtime_1.jsx)("label", { className: "block text-xs font-medium uppercase tracking-wide text-slate-400", children: "Date" }) }), (0, jsx_runtime_1.jsxs)("div", { className: "md:flex md:items-center md:justify-end md:gap-3", children: [(0, jsx_runtime_1.jsx)("div", { className: "md:w-[150px]", children: datePicker }), (0, jsx_runtime_1.jsx)("div", { className: "mt-3 md:mt-0 md:w-[150px]", children: dayOfWeekSelect })] })] }) })] }), !showMyReceipts && ((0, jsx_runtime_1.jsxs)("div", { className: "grid grid-cols-1 gap-6 lg:grid-cols-12", children: [(0, jsx_runtime_1.jsxs)("div", { className: "space-y-6 lg:col-span-8", children: [(0, jsx_runtime_1.jsx)(SalesReceiptsCard, { receipts: receipts, updateReceipt: updateReceipt, updateProduct: updateProduct, addProductToReceipt: addProductToReceipt, removeProductFromReceipt: removeProductFromReceipt, addReceipt: addReceipt, removeReceipt: removeReceipt, salesTotals: salesTotals }), (0, jsx_runtime_1.jsxs)("div", { className: "space-y-6", children: [(0, jsx_runtime_1.jsx)(DaySpecificBlocks, { selectedDay: dayOfWeek, walkIns: Number(walkinsServed || 0), onWalkInsChange: (val) => setWalkinsServed(val), neatness: shopNeatness, onNeatnessChange: setShopNeatness, productTasks: {
                                        ...
                                        }, communications: communications, onCommunicationsChange: setCommunications, marketplace: marketplace, onMarketplaceChange: setMarketplace, liveSession: liveSession, onLiveSessionChange: setLiveSession, thursdayActivities: thursdayActivities, onThursdayActivitiesChange: setThursdayActivities, fridayTasks: fridayTasks, onFridayTasksChange: setFridayTasks, saturdaySummary: saturdaySummary, onSaturdaySummaryChange: setSaturdaySummary }), (0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col gap-3 sm:flex-row sm:justify-end", children: [(0, jsx_runtime_1.jsx)("button", { type: "button", onClick: handleResetDay, className: "rounded-xl border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-white/5", children: "Reset day" }), (0, jsx_runtime_1.jsx)("button", { type: "button", onClick: handleSubmit, disabled: isSubmitting, className: "rounded-xl bg-emerald-500 px-6 py-2 text-sm font-semibold text-black hover:brightness-95 disabled:opacity-60", children: isSubmitting ? "Submitting..." : "Submit report" })] })] })] }), (0, jsx_runtime_1.jsxs)("div", { className: "space-y-6 lg:col-span-4", children: [(0, jsx_runtime_1.jsx)(QuickStats, { receipts: displayedReceipts, salesKes: displayedSalesKes, newProducts: displayedNewProducts, editedProducts: displayedEditedProducts, copiedProducts: displayedCopiedProducts, walkInsServed: displayedWalkInsServed, walkInsPurchased: displayedWalkInsPurchased, commissionKes: commissionForPeriod, periodLabel: selectedPeriodLabel }), (0, jsx_runtime_1.jsx)(EarningsCard_1.default, { summary: earningsSummary ?? publicFallbackSummary, lockKey: "dailyreport:earnings" })] })] })), showMyReceipts && (0, jsx_runtime_1.jsx)(daily_report_receipts_1.default, { start: date, end: date, attendantId: attendantId })] }));
}
