...
function renderReceiptTemplate(snapshot) {
    ...
    const itemsHtml = items.map((it) => `<tr><td>${(it.title || it.productName || '')}</td><td style="text-align:right">${it.quantity || 1}</td><td style="text-align:right">${it.unitPrice || it.sellingPrice || ''}</td><td>${it.serial || ''}</td><td>${it.warranty || ''}</td></tr>`).join('');
    return `
  <!doctype html>
  <html>
  <head>
