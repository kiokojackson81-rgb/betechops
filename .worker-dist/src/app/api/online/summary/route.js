...
async function GET(req) {
    ...
    const { assignments, accountIds } = await (0, onlineOps_1.getMarketplaceAssignmentsForUser)(targetUserId);
    if (!accountIds.length) {
        ...
        return server_1.NextResponse.json((0, resolveTargetUser_1.composeIdentityResponse)(meta, emptyData));
    }
    ...
    let payoutSales = 0;
    if (accountIds.length) {
        const aggs = await (0, recomputeWeeklySummaries_1.recomputeWeeklySummary)(start, end);
        const filtered = aggs.filter((a) => accountIds.includes(a.accountId));
        payoutSales = filtered.reduce((s, a) => s + Number(a.totalGross ?? 0), 0);
    }
    const manualSummary = await prisma_1.prisma.weeklySale.aggregate({
        _sum: { amount: true },
    ...
    const commissionInfo = (0, marketingCommission_1.getCommissionSummaryForSales)(marketplaceSalesOnly);
