...
async function POST(req, context) {
    const guard = await (0, api_2.requireRole)(['ADMIN']);
    if (!guard.ok)
        return guard.res;
    ...
    try {
        ...
        const actorId = guard.session?.user?.id ?? (await (0, api_1.getActorId)());
        await prisma_1.prisma.actionLog.create({ data: { actorId: actorId, entity: 'Balance', entityId: userId, action: 'ADJUST', before: client_1.Prisma.JsonNull, after: bal } });
