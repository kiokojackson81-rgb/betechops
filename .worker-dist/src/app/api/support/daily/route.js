...
async function POST(req) {
    ...
    for (const receipt of normalizedReceipts) {
        const normalized = (0, receiptGuard_1.canonicalReceiptNumber)(receipt.receiptNumber || undefined);
        if (!normalized)
            continue;
        if (seenReceipts.has(normalized)) {
            return server_1.NextResponse.json({ error: `Duplicate receipt ${normalized} in submission` }, { status: 409 });
        }
    ...
    try {
        ...
        try {
            ...
            if (adminPhone) {
                const { sendWhatsAppTextMessage } = await Promise.resolve().then(() => __importStar(require('@/lib/notifications/whatsapp')));
                const actorUser = await prisma_1.prisma.user.findUnique({ where: { id: auth.user.id }, select: { name: true, email: true } }).catch(() => null);
