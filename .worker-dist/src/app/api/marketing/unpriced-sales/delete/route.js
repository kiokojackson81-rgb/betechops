...
async function POST(req) {
    ...
    try {
        if (source === "daily-sale") {
            ...
            await prisma_1.prisma.$transaction(async (tx) => {
                if (sale.receiptNumber) {
                    await (0, marketingReceiptCleanup_1.cleanupMarketingReceipts)(tx, sale.receiptNumber);
                    await (0, marketingReceiptCleanup_1.cleanupSupportReceipts)(tx, sale.receiptNumber);
                    await (0, marketingReceiptCleanup_1.deleteReceiptOrderCascade)(tx, sale.receiptNumber).catch(() => { });
                }
        ...
        await prisma_1.prisma.$transaction(async (tx) => {
            await (0, marketingReceiptCleanup_1.cleanupSupportReceipts)(tx, item.receipt?.receiptNumber ?? undefined, item.receiptId);
            if (item.receipt?.receiptNumber) {
                await (0, marketingReceiptCleanup_1.cleanupMarketingReceipts)(tx, item.receipt.receiptNumber);
                await (0, marketingReceiptCleanup_1.deleteReceiptOrderCascade)(tx, item.receipt.receiptNumber).catch(() => { });
            }
            else {
                await (0, marketingReceiptCleanup_1.cleanupMarketingReceipts)(tx, undefined, item.receiptId);
            }
        });
