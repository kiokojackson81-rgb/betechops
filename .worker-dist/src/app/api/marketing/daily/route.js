...
async function POST(req) {
    ...
    for (const receipt of receiptsClean) {
        const normalized = (0, receiptGuard_1.canonicalReceiptNumber)(receipt.receiptNumber);
        if (!normalized)
            continue;
    ...
    try {
        ...
        const commissionInfo = (0, marketingCommission_1.getCommissionSummaryForSales)(periodSales);
        let commissionValue = 0;
        if (periodProfit > 0) {
            commissionValue = commissionInfo.commission ?? 0;
            if (commissionValue === 0 && periodSales > 0 && periodSales < 500000) {
