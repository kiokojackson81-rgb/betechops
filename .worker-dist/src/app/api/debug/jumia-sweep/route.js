...
async function GET(req) {
    ...
    if (url.searchParams.get('run') !== 'true') {
        return server_1.NextResponse.json({ ok: false, message: 'Call with ?run=true to execute sweep' });
    }
        ...
        for (const base of bases) {
            ...
            if (token) {
                for (const scheme of authSchemes) {
                    try {
                        const headers = { Authorization: `${scheme} ${token}`, Accept: 'application/json' };
    ...
    return server_1.NextResponse.json({ ok: true, results, timestamp: new Date().toISOString() }, { status: 200 });
}
