...
async function POST(req) {
    ...
    for (const s of shops) {
        ...
        try {
            const existing = await prisma_1.prisma.shop.findFirst({ where: { name } });
            const data = {
                name,
            ...
            if (existing) {
                const updated = await prisma_1.prisma.shop.update({ where: { id: existing.id }, data });
                results.push({ name, action: "updated", id: updated.id });
            }
            else {
                const created = await prisma_1.prisma.shop.create({ data });
