...
async function fetchNewestForShop(shopId, params) {
    ...
    if (params.country)
        qs.push(`country=${encodeURIComponent(params.country)}`);
    if (params.dateFrom)
        qs.push(`createdAfter=${encodeURIComponent(params.dateFrom)}`);
    if (params.dateTo)
        qs.push(`createdBefore=${encodeURIComponent(params.dateTo)}`);
...
async function GET(req) {
    ...
    if (shopIdParam && shopIdParam.toUpperCase() !== 'ALL') {
        ...
        if (!shopIds.length) {
            ...
            const def = await (0, jumia_1.loadDefaultShopAuth)().catch(() => undefined);
            if (!def)
                shopIds = [];
        }
    }
