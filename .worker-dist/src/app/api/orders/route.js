...
async function GET(req) {
    ...
    if (process.env.NODE_ENV === 'test') {
        const isMock = jumiaFetch?._isMockFunction === true || typeof jumiaFetch.mock === 'function';
        if (!isMock) {
    ...
    if (!globalThis.__ordersPendingCache) {
        globalThis.__ordersPendingCache = new Map();
    }
    ...
    try {
        ...
        if ((qs.shopId || '').toUpperCase() === 'ALL') {
            ...
            const getId = (x) => String(x?.id ?? x?.number ?? x?.orderNumber ?? '');
            const cmpDesc = (a, b) => {
            ...
            const states = await Promise.all(jumiaShops.map(async (s) => {
                ...
                try {
                    ...
                    const arr = Array.isArray(j?.orders)
                        ...
                        : Array.isArray(j?.items)
                            ...
                            : Array.isArray(j?.data)
                                ? j.data
                                : [];
                    const annotated = arr.map((o) => (o && typeof o === 'object' ? { ...o, shopIds: (o?.shopIds?.length ? o.shopIds : [s.id]) } : o));
                    st.token = String(j?.nextToken ?? j?.token ?? '') || null;
                    st.isLast = !st.token;
                    ...
                    while (cursor && st.buf.length === 0 && !st.isLast && safety < 5) {
                        ...
                        const arr2 = Array.isArray(j2?.orders)
                            ? j2.orders
                            : Array.isArray(j2?.items)
                                ...
                                : Array.isArray(j2?.data)
                                    ? j2.data
                                    : [];
                        const annotated2 = arr2.map((o) => (o && typeof o === 'object' ? { ...o, shopIds: (o?.shopIds?.length ? o.shopIds : [s.id]) } : o));
                        st.token = String(j2?.nextToken ?? j2?.token ?? '') || null;
                        st.isLast = !st.token;
                            ...
                            const arr = Array.isArray(j?.orders)
                                ...
                                : Array.isArray(j?.items)
                                    ...
                                    : Array.isArray(j?.data)
                                        ? j.data
                                        : [];
                            const annotated = arr.map((o) => (o && typeof o === 'object' ? { ...o, shopIds: (o?.shopIds?.length ? o.shopIds : [st.id]) } : o));
                            st.token = String(j?.nextToken ?? j?.token ?? '') || null;
                            st.isLast = !st.token;
