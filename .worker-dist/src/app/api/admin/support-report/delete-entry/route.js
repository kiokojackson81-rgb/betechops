...
async function POST(req) {
    ...
    try {
        const before = await prisma_1.prisma.supportDailyEntry.findUnique({ where: { id: entryId }, include: { receipts: { include: { items: true } } } });
        if (!before)
            return server_1.NextResponse.json({ error: "Entry not found" }, { status: 404 });
        // delete items, receipts, then entry
        await prisma_1.prisma.supportReceiptItem.deleteMany({ where: { receipt: { is: { dailyEntryId: entryId } } } });
        await prisma_1.prisma.supportReceipt.deleteMany({ where: { dailyEntryId: entryId } });
        await prisma_1.prisma.supportDailyEntry.delete({ where: { id: entryId } });
        // audit
        try {
