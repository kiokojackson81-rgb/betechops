...
async function GET(req) {
    ...
    if (periodKeyParam) {
        ...
        if (parts.length === 2) {
            ...
            period = (0, tradingPeriod_1.getTradingPeriodFor)(new Date());
        }
    }
    ...
    const rows = attendants.map((attendant) => {
        ...
        const ledgerCommissionTotal = Number(ledger?.commissionTotal ?? ledger?.netCommission ?? ledger?.grossCommission ?? 0);
