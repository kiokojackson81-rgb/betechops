...
const marketplaceAssignment_1 = require("@/lib/marketplaceAssignment");
exports.dynamic = "force-dynamic";
async function GET() {
    const guard = await (0, api_1.requireRole)(["ADMIN", "SUPERVISOR"]);
    if (!guard.ok)
        return guard.res;
    const now = new Date();
    const [accounts, shops] = await Promise.all([
        prisma_1.prisma.marketplaceAccount.findMany({
            where: { isActive: true },
            orderBy: { displayName: "asc" },
            include: {
                assignments: {
                    where: {
                        role: { in: marketplaceAssignment_1.MarketplaceAssignmentRoleValues },
    ...
    const shopsByName = new Map(shops
    ...
    const shopsByApiKey = new Map();
    for (const shop of shops) {
        const apiKey = shop.apiConfig?.apiKey;
        if (apiKey)
            shopsByApiKey.set(String(apiKey), shop);
    }
    ...
    const payload = accounts
        ...
        const attendants = account.assignments
            ...
            .filter((attendant) => Boolean(attendant))
            .map((attendant) => ({
            id: attendant.id,
    ...
    const typedEmptyAttendant = null;
    ...
    return server_1.NextResponse.json(finalPayload);
}
