...
async function GET(req) {
    ...
    if (!resp.ok)
        return server_1.NextResponse.json({ error: 'Failed to fetch reports from API' }, { status: 500 });
    const data = await resp.json();
    let puppeteer;
    try {
        // dynamic import so route still works if puppeteer is not installed
        const mod = await Promise.resolve().then(() => __importStar(require('puppeteer'))).catch(() => null);
        puppeteer = (mod && mod.default) ? mod.default : mod;
        if (!puppeteer) {
            return server_1.NextResponse.json({ error: 'Server-side PDF generation requires `puppeteer`. Install it or use client-side print.' }, { status: 501 });
        }
    }
    catch (err) {
        return server_1.NextResponse.json({ error: 'Server-side PDF generation requires `puppeteer`. Install it or use client-side print.' }, { status: 501 });
    ...
    try {
        ...
        await browser.close();
        return new server_1.NextResponse(pdfBuffer, { status: 200, headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="daily_reports.pdf"' } });
    }
    catch (err) {
        if (browser)
            await browser.close();
        return server_1.NextResponse.json({ error: 'Failed to generate PDF', detail: String(err) }, { status: 500 });
    }
}
