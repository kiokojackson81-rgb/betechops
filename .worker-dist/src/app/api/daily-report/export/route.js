...
async function GET(req) {
    try {
        ...
        if (toStr)
            where.date = where.date ? { ...where.date, lte: new Date(toStr) } : { lte: new Date(toStr) };
        if (day)
            where.day = day;
        const pageSize = 1000;
                ...
                while (true) {
                    ...
                    for (const r of rows) {
                        ...
                        const fields = [
                            ...
                            String((tasks.dayFields || {}).videosParticipated ?? ''),
                            ...
                            JSON.stringify(tasks.marketplaceReview ?? {}),
