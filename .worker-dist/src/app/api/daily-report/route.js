...
async function POST(req) {
    ...
    try {
        ...
        for (const sale of taskSales) {
            const normalized = (0, receiptGuard_1.canonicalReceiptNumber)(typeof sale?.receiptNumber === "string" ? sale.receiptNumber : "");
            if (!normalized)
                continue;
        ...
        if (hasProfitValue) {
            ...
            delete tasksWithSubmit.metrics.totalProfit;
        }
        const reportPayload = {
            date: reportDate,
            day: String(day),
