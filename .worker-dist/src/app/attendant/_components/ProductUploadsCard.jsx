...
function ProductUploadsCard() {
    ...
    return (<section className="rounded-2xl border border-cyan-400/20 bg-[linear-gradient(135deg,rgba(9,33,38,.95),rgba(9,33,38,.75))] p-4 shadow-lg shadow-cyan-900/30">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
        <div>
          ...
          <input value={count} onChange={(e) => setCount(e.target.value)} type="number" min="0" step="1" placeholder="e.g. 18" className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-cyan-400"/>
        </div>
        <button onClick={submit} disabled={busy} className="self-end rounded-lg bg-cyan-500/80 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-500 focus:outline-none disabled:opacity-50">
      ...
      <div className="mt-3">
        ...
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} maxLength={200} rows={2} placeholder="Optional: account/channel or next steps" className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-cyan-300"/>
      </div>

      <div className="mt-4">
        <h3 className="text-xs uppercase tracking-widest text-slate-400">Last submissions</h3>
        <ul className="mt-2 space-y-2">
          {history.map((row) => (<li key={row.id} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm">
              <div>
          ...
          {!history.length && (<li className="rounded-lg border border-dashed border-white/10 px-3 py-4 text-center text-xs text-slate-500">No uploads logged yet</li>)}
        </ul>
      </div>
    </section>);
}
