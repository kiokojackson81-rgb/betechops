...
async function fetchSyncedRows(params) {
    ...
    try {
        const missingShopIds = Array.from(new Set(orders
            .map((o) => o.shopId)
            .filter((s) => s && (!orders.find((x) => x.shopId === s)?.shop || !orders.find((x) => x.shopId === s)?.shop?.name))));
        if (missingShopIds.length) {
            const resolved = await prisma_1.prisma.jumiaShop.findMany({
                where: { id: { in: missingShopIds } },
                select: { id: true, name: true, account: { select: { label: true } } },
            }).catch(() => []);
            const byId = new Map();
            for (const r of resolved)
                byId.set(r.id, { name: r.name, accountLabel: r.account?.label ?? null });
