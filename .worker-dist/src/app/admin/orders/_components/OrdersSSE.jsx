...
function OrdersSSE({ status, country, shopId, dateFrom, dateTo, intervalMs = 4000 }) {
    ...
    return (<div className="text-xs text-slate-400 flex items-center gap-2" title="Live updates via SSE">
      <span>Live</span>
      <span className={live === 'on' ? 'inline-block w-2 h-2 rounded-full bg-green-500' : live === 'connecting' ? 'inline-block w-2 h-2 rounded-full bg-yellow-500' : 'inline-block w-2 h-2 rounded-full bg-slate-500'}/>
