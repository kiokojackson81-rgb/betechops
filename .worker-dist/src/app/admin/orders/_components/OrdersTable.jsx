...
function OrdersTable({ rows, nextToken, isLastPage }) {
    ...
    return (<div className="rounded-xl border border-white/10 bg-[var(--panel,#121723)] overflow-auto">
      {someSelected && (<div className="flex items-center justify-between gap-3 p-3 border-b border-white/10 bg-black/20 sticky top-0 z-10">
          <div className="text-sm">Selected {selectedIds.length} row(s)</div>
          <div className="flex items-center gap-2">
            <button className="px-3 py-1 rounded border border-white/10 hover:bg-white/10 disabled:opacity-50" onClick={() => runBulk("pack")} disabled={!!bulkBusy}>
              {bulkBusy === "pack" ? "Packing…" : "Pack selected"}
            </button>
            <button className="px-3 py-1 rounded border border-white/10 hover:bg-white/10 disabled:opacity-50" onClick={() => runBulk("rts")} disabled={!!bulkBusy}>
              {bulkBusy === "rts" ? "Marking…" : "RTS selected"}
            </button>
            <button className="px-3 py-1 rounded border border-white/10 hover:bg-white/10 disabled:opacity-50" onClick={() => runBulk("print")} disabled={!!bulkBusy}>
              {bulkBusy === "print" ? "Printing…" : "Print selected"}
            </button>
            <button className="px-3 py-1 rounded border border-white/10 hover:bg-white/10" onClick={() => setSelected({})}>
              Clear
            </button>
          </div>
        </div>)}
          ...
          {rows.map((row) => {
            ...
            return (<tr key={row.id} className="border-t border-white/5">
                <td className="px-3 py-2">
                  <input type="checkbox" checked={!!selected[row.id]} onChange={() => toggleRow(row.id, row.shopId || (Array.isArray(row.shopIds) ? row.shopIds[0] : undefined))}/>
                </td>
                ...
                <td className="px-3 py-2">
                  {details[row.id]?.url ? (<a href={details[row.id]?.url ?? "#"} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:underline" title={details[row.id]?.name || "Open product on Jumia"}>
                      {(row.shopName ?? row.shopId ?? row.shopIds?.[0] ?? "-")}
                      {(details[row.id]?.count ?? 0) > 1 && (<span className="ml-1 text-xs opacity-70">(x{details[row.id]?.count})</span>)}
                    </a>) : (<>
                      {row.shopName ?? row.shopId ?? row.shopIds?.[0] ?? "-"}
                      {(details[row.id]?.count ?? 0) > 1 && (<span className="ml-1 text-xs opacity-70">(x{details[row.id]?.count})</span>)}
                    </>)}
