...
async function JumiaReturnsPage({ searchParams, }) {
    ...
    return (<div className="p-6 space-y-4">
      ...
      <form className="flex flex-wrap gap-2 items-end">
        <div>
          ...
          <select name="shopId" defaultValue={shopId} className="rounded bg-white/5 border border-white/10 px-2 py-1.5">
            ...
            {(await prisma_1.prisma.shop.findMany({ where: { isActive: true, platform: "JUMIA" }, select: { id: true, name: true }, orderBy: { name: "asc" } }))
      ...
      <div className="text-xs text-slate-400">
        ...
        {nextToken && (<a className="ml-2 underline" href={`/admin/returns/jumia?${qs({ size, status, token: nextToken, shopId })}`}>Next →</a>)}
      </div>
    </div>);
}
