...
function CountsRefreshButton({ shopId, exact }) {
    ...
    if (exact)
        qs.set("exact", "true");
    return ((0, jsx_runtime_1.jsx)("button", { disabled: loading, onClick: async () => {
            try {
                setLoading(true);
        ...
        }, className: `rounded border px-3 py-1 text-xs ${loading ? "border-white/10 bg-white/10 text-slate-300" : "border-white/15 bg-white/5 text-slate-100 hover:border-white/25"}`, title: "Refresh counts now", children: loading ? "Refreshing…" : "Refresh counts" }));
}
