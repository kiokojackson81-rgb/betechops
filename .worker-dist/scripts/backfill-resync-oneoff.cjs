...
const { prisma: prismaWorkerResync } = require('../src/lib/prisma.js');
...
;(async () => {
  async function main() {
    ...
    const missing = await prismaWorkerResync.jumiaOrder.count({ where: { OR: [ { totalAmountLocalValue: null }, { totalAmountLocalCurrency: null } ] } });
    console.log('[worker-resync-oneoff] remaining missing totals rows=' + missing);
    await prismaWorkerResync.$disconnect();
  }

  try {
    await main();
  } catch (err) {
    console.error('[worker-resync-oneoff] fatal', err);
    process.exit(1);
  }
})();