...
async function main() {
    ...
    const limit = (0, p_limit_1.default)(Math.max(1, Math.min(6, concurrency)));
    ...
    const results = await Promise.all(shops.map((s) => limit(async () => {
        try {
            const recent = await prisma_1.prisma.jumiaOrder.findFirst({ where: { shopId: s.id }, orderBy: { updatedAt: 'desc' }, select: { id: true } });
            if (!recent)
                return { shopId: s.id, shopName: s.name, orderId: null, sampleOrderItemId: null, providers: [], error: 'noOrders' };
            ...
            if (!firstItemId)
                return { shopId: s.id, shopName: s.name, orderId: recent.id, sampleOrderItemId: null, providers: [], error: 'noItems' };
    ...
    for (const r of results) {
        const names = r.providers.map((p) => `${p.name || p.id}${p.requiresTracking ? ' (tracking)' : ''}`);
