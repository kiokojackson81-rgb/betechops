...
function calculateCumulativeCommission(totalSales) {
  ...
  return { commission, nextTarget: nextTier ? nextTier.min : null, tiersReached: tiersReached.map((t) => `${t.min}`), nextTierReward: nextTier ? nextTier.reward : null };
}
...
async function main() {
  ...
  try {
    ...
    const [marketingEntries, reports] = await Promise.all([
      prisma.marketingDailyEntry.findMany({ where: { submittedById: user.id, date: { gte: period.start, lte: period.end } }, include: { receipts: { include: { items: true } }, sales: true } }),
    ...
    if (totals.totalProfit > 0) {
      ...
      const fallbackCommission = baseCommission === 0 && totals.totalSales > 0 && totals.totalSales < 500000 ? Math.round(Math.max(totals.totalProfit, 0) * 0.05) : 0;
    ...
    const previousDetail = existingLedger && existingLedger.detail ? existingLedger.detail : {};
    const previousMarketing = previousDetail && previousDetail.marketing ? previousDetail.marketing : null;
    const previousMarketingCommission = toNumber(previousMarketing && previousMarketing.commission ? previousMarketing.commission : 0);
    const grossBeforeMarketing = Math.max(0, toNumber(existingLedger && existingLedger.grossCommission ? existingLedger.grossCommission : 0) - previousMarketingCommission);
    const grossCommission = grossBeforeMarketing + marketingCommission;
    const penalties = toNumber(existingLedger && existingLedger.penalties ? existingLedger.penalties : 0);
    const netCommission = grossCommission - penalties;

    const nextDetail = Object.assign({}, existingLedger && existingLedger.detail ? existingLedger.detail : {}, { marketing: { periodKey: period.key, totals: totals, commission: marketingCommission, computedAt: new Date().toISOString() } });
    ...
    const ledger = await prisma.commissionLedger.upsert({
      ...
      update: {
        ...
        commissionTotal: (Number(existingLedger && existingLedger.commissionTotal ? existingLedger.commissionTotal : 0) - previousMarketingCommission + marketingCommission).toFixed(2),
        detail: nextDetail,
      },
      create: {
        userId: user.id,
