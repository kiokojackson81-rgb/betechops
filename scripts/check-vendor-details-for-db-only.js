...
(async function main() {
  try {
    ...
    const windowDays = Number(process.argv[3] || 90);
    const samplePerShop = Number(process.argv[4] || 10);
    ...
    for (const account of accounts) {
      try {
        ...
        for (const rs of remoteShops) {
          ...
          // For a sample of dbOnly IDs, call vendor detail endpoint
          const sample = dbOnly.slice(0, samplePerShop);
          if (!sample.length) continue;
          console.log(`\nFetching vendor detail for up to ${sample.length} DB-only orders:`);
          for (const id of sample) {
            // try common patterns
            const attempts = [
              `/orders/${encodeURIComponent(id)}`,
              `/orders/${encodeURIComponent(id)}?shopId=${encodeURIComponent(shopId)}`,
              `/orders?orderId=${encodeURIComponent(id)}`,
              `/orders?order_id=${encodeURIComponent(id)}`,
            ];
            let got = false;
            for (const p of attempts) {
              try {
                const res = await call(account.credentials?.apiBase || 'https://vendor-api.jumia.com', p, token);
                console.log(`-> Detail ${p} => status=${res.status} body=${JSON.stringify(res.body).slice(0,500)}`);
                if (res.status >= 200 && res.status < 300) { got = true; break; }
                // small delay to avoid rate bursts
                await new Promise(r => setTimeout(r, 150));
              } catch (e) {
                console.error(`error calling ${p}:`, e.message || e);
              }
            }
            if (!got) console.log(`No successful detail for order ${id}`);
          }
        }
      } catch (e) {
