...
async function run() {
  ...
  try {
    ...
    const table = await prisma.$queryRawUnsafe(
      "select exists (select 1 from information_schema.tables where table_schema='public' and table_name='CatalogCounters') as exists;"
    );
    ...
    if (table && table[0] && table[0].exists) {
      ...
      const latest = await prisma.$queryRawUnsafe(
        'select scope, "shopId", total, active, "qcApproved", approx, "computedAt" from "CatalogCounters" order by "computedAt" desc limit 10;'
      );
