...
async function inspectDbOnlyDetails(shopId, dbOnlyIds = []) {
  if (!dbOnlyIds || !dbOnlyIds.length) return [];
  const rows = await prisma.jumiaOrder.findMany({ where: { id: { in: dbOnlyIds } }, select: {
    id: true, status: true, shopId: true, createdAt: true, updatedAt: true, createdAtJumia: true, updatedAtJumia: true
    ...
    for (const account of accounts) {
      try {
        ...
        for (const rs of remoteShops) {
          ...
          if (dbOnly.length) {
            console.log('\nFetching details for a sample of DB-only rows (up to 50):');
            const details = await inspectDbOnlyDetails(shopId, dbOnly.slice(0, 200));
