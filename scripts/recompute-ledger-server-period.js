...
// Compute trading period using UTC midnight boundaries (matches servers running in UTC)
function getTradingPeriodForUTC(date) {
  const d = new Date(date);
  // use UTC year/month/day
...
async function main() {
  ...
  console.log(`Recompute ledger (server-period UTC) for ${email}`);
  ...
  try {
    ...
    const now = new Date();
    const period = getTradingPeriodForUTC(now);

    // load tiers if created in DB
    ...
    if (periodRow) {
      ...
      if (rows && rows.length > 0) tiers = rows.map((r) => ({ minSales: Number(r.minSales), maxSales: Number(r.maxSales), payoutFlat: Number(r.payoutFlat) }));
    }

    const snapshots = await prisma.profitSnapshot.findMany({ where: { orderItem: { order: { attendantId: user.id, createdAt: { gte: period.start, lte: period.end } } } }, select: { revenue: true, profit: true } });
    ...
    const previousMarketingCommission = toNumber(existingDetail.marketing?.commission);
    ...
    const ledger = await prisma.commissionLedger.upsert({ where: { userId_periodStart_periodEnd: { userId: user.id, periodStart: period.start, periodEnd: period.end } }, update: { grossCommission: grossCommissionNew.toFixed(2), netCommission: netCommission.toFixed(2), commissionTotal: (Number(existing?.commissionTotal ?? 0) - previousMarketingCommission + salesCommission).toFixed(2), detail: nextDetail }, create: { userId: user.id, periodStart: period.start, periodEnd: period.end, grossCommission: grossCommissionNew.toFixed(2), netCommission: netCommission.toFixed(2), commissionTotal: salesCommission.toFixed(2), detail: nextDetail } });

    console.log('Upserted ledger id:', ledger.id, 'commissionTotal:', ledger.commissionTotal);
    console.log('Ledger periodStart (UTC):', ledger.periodStart.toISOString());
