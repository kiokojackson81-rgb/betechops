...
async function main() {
  ...
  console.log('Scanning receipts for', userId, 'period', period.label);
  ...
  for (const [k,v] of map) { totalSelling += v.selling; totalProfit += (v.selling - v.buying); if (v.selling - v.buying !== 0) console.log('receipt', k, 'selling', v.selling, 'buying', v.buying, 'profit', v.selling - v.buying, 'sources', Array.from(v.sources).join(',')); }

  console.log('TotalSelling', totalSelling, 'TotalProfit', totalProfit);
