...
async function main() {
  ...
  for (let i = 0; i < weeks; i++) {
    ...
    // find payout rows that overlap this week window
    ...
    results.push({ weekStart: weekStart.toISOString().split('T')[0], weekEnd: weekEnd.toISOString().split('T')[0], rows: rows.length, totalGross, totalPayout, weeklySum, duplicates: duplicates.length, missingSids: missingSids.size });
  }
