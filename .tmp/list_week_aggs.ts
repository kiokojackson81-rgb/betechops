...
async function main() {
  ...
  for (const a of aggs) {
    ...
    out.push({ accountId: a.accountId, displayName: acct?.displayName ?? null, totalPayout: a.totalPayout, totalGross: a.totalGross });
  }

  fs.mkdirSync('.tmp', { recursive: true });
  fs.writeFileSync('.tmp/week_2025-12-29_aggs.json', JSON.stringify({ windowStart: start.toISOString(), windowEnd: end.toISOString(), aggs: out }, null, 2));
  console.log(`Wrote .tmp/week_2025-12-29_aggs.json — entries: ${out.length}`);

  await prisma.$disconnect().catch(() => undefined);
}

main().catch((e) => {
  console.error('list_week_aggs failed:', e);
  process.exit(1);
});
