...
async function fetchOrderItems(apiBase, authHeader, orderId, shopId) {
...
(async () => {
  try {
    ...
    try {
      const items = await fetchOrderItems(apiBase, auth, matched.id, account.jumiaShopSid);
      console.log('items count for UUID:', items.length);
      if (items.length) console.dir(items, { depth: null });
    } catch (err) {
      console.error('fetchOrderItems error:', err.message || err);
    }
  } catch (err) {
