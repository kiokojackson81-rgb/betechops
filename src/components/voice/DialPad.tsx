...
function DialPadInner({
  ...
  return (
    <div className={`rounded-[24px] border border-slate-800/80 bg-slate-950/92 p-4 ${compact ? "space-y-4" : ""}`}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs font-medium text-slate-400">Dial pad</div>
        ...
        <button
          ...
          className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-800 bg-slate-900/80 text-slate-100 transition hover:border-slate-600"
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {DIGITS.map((digit) => (
          <button
            ...
            className="rounded-2xl border border-slate-800 bg-slate-900/80 px-3 py-4 text-lg font-semibold text-white transition hover:border-cyan-400/40 hover:bg-cyan-500/10"
          >
            {digit}
          </button>
        ))}
          ...
          <div className="mt-3 space-y-2">
            {softphone.favoriteNumbers.map((favorite) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-2xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-left transition hover:border-slate-600"
              >
          ...
          <div className="mt-3 space-y-2">
            {softphone.recentCalls.slice(0, 4).length ? softphone.recentCalls.slice(0, 4).map((call) => (
              ...
              <div className="rounded-2xl border border-dashed border-slate-800 px-3 py-4 text-sm text-slate-500">
                Recent browser and operator calls will appear here.
              </div>
            )}
          </div>
        </div>
