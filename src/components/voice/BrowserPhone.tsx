...
import { ChevronDown, ChevronUp, RefreshCw, RadioTower, Settings2, TestTube2 } from "lucide-react";
...
export default function BrowserPhone() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[80] w-[min(100vw-1.5rem,430px)]">
      <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.97),rgba(2,6,23,0.98))] shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
          <div className="min-w-0">
            <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Browser softphone</div>
            <div className="mt-1 truncate text-sm font-semibold text-white">
              {softphone.currentCall ? `${softphone.currentCall.displayName} · ${softphone.currentCall.remoteIdentity}` : "Mock-ready operator console"}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-label={softphone.isCollapsed ? "Expand softphone" : "Collapse softphone"}
            >
