...
export async function generateSiteAssessmentReportPdf(input: {
  ...
  const calculation = asRecord(assessment.calculation);
  ...
  const connectedKw = finite(numeric(calculation.connectedKw), input.report.calculation.connectedKw);
  const dailyKwh = finite(numeric(calculation.dailyKwh), input.report.calculation.dailyKwh);
  const simultaneousPeakKw = finite(numeric(calculation.simultaneousPeakKw), connectedKw);
  const inverterKw = finite(numeric(calculation.inverterKw), input.report.calculation.inverterKw);
  const batteryKwh = finite(numeric(calculation.batteryKwh), input.report.calculation.batteryKwh);
  const pvKw = finite(numeric(calculation.pvKw), input.report.calculation.pvKw);
  const panelCount = Math.max(0, Math.round(finite(numeric(calculation.panelCount), input.report.calculation.panelCount)));
  const backupHours = finite(numeric(electrical.backupHours), 0);
  const monthlyKwh = dailyKwh * 30;
  const annualKwh = dailyKwh * 365;
  const essentialKw = loads.filter((load) => Boolean(load.essential)).reduce((total, load) => total + numeric(load.qty) * numeric(load.watts) / 1000, 0);
  const dayKwh = loads.filter((load) => ["Day", "Both"].includes(clean(load.period))).reduce((total, load) => total + loadEnergyWh(load) / 1000, 0);
  const nightKwh = loads.filter((load) => ["Night", "Both"].includes(clean(load.period))).reduce((total, load) => total + loadEnergyWh(load) / 1000, 0);
  const alwaysOnKw = loads.filter((load) => clean(load.usageMode) === "ALWAYS_ON").reduce((total, load) => total + numeric(load.qty) * numeric(load.watts) / 1000, 0);
  const topLoads = [...loads].sort((a, b) => loadEnergyWh(b) - loadEnergyWh(a)).slice(0, 3).filter((load) => loadEnergyWh(load) > 0);
  ...
  y -= 43;
  const status = input.report.signatures ? "DIGITALLY ACKNOWLEDGED" : "TECHNICALLY ASSESSED";
  ...
  const metrics = [
    ...
    ["Indicative PV array", `${formatNumber(pvKw)} kWp`, `${panelCount || "Indicative"} × 600W panels.`],
  ];
  ...
  const conclusion = input.report.aiReview?.summary || `The property has an estimated connected electrical load of ${formatNumber(connectedKw)} kW and a practical simultaneous demand of approximately ${formatNumber(simultaneousPeakKw)} kW. Recorded appliance usage indicates approximately ${formatNumber(dailyKwh)} kWh per day, equivalent to ${formatNumber(monthlyKwh, 0)} kWh per month. For the presently recorded loads, Betech Solar Solutions recommends a hybrid inverter in the ${formatNumber(inverterKw, 1)} kW class with an indicative solar PV array of approximately ${formatNumber(pvKw)} kWp. Final battery capacity follows the selected backup loads and required backup period.`;
  const conclusionHeight = Math.max(78, drawParagraph(page, conclusion, MARGIN + 14, y - 20, A4[0] - MARGIN * 2 - 28, regular, 9, INK, 13) + 30);
  ...
  loads.forEach((load, index) => {
    ...
    const energy = loadEnergyWh(load) / 1000;
  ...
  const inverterHeight = card(MARGIN, designWidth, "Hybrid inverter", [["Recorded simultaneous peak", `${formatNumber(simultaneousPeakKw)} kW`], ["Design headroom", "25% operating reserve"], ["Recommended standard size", `${formatNumber(inverterKw, 1)} kW hybrid inverter`], ["Status", inverterKw >= simultaneousPeakKw * 1.1 ? "SUITABLE" : "TECHNICAL REVIEW REQUIRED"]], inverterKw >= simultaneousPeakKw * 1.1 ? "plain" : "amber");
  const batteryHeight = card(MARGIN + designWidth + 12, designWidth, "Battery storage", [["Required backup", backupHours ? `${formatNumber(backupHours, 1)} hours` : ""], ["Essential load", essentialKw ? `${formatNumber(essentialKw)} kW` : ""], ["Recommended storage", batteryKwh ? `${formatNumber(batteryKwh)} kWh lithium` : ""], ["Status", batteryKwh && backupHours ? "PRELIMINARY DESIGN" : "BACKUP LOAD CONFIRMATION REQUIRED"]], batteryKwh && backupHours ? "plain" : "amber");
  y -= Math.max(inverterHeight, batteryHeight) + 13;
  const pvHeight = card(MARGIN, designWidth, "Solar PV array", [["Daily energy", `${formatNumber(dailyKwh)} kWh/day`], ["Peak sun hours", "4.5 h/day"], ["Performance factor", "78%"], ["Recommended practical array", `${formatNumber(pvKw)} kWp`], ["Indicative panels", panelCount ? `${panelCount} × 600W` : ""]]);
  const configurationHeight = card(MARGIN + designWidth + 12, designWidth, "System configuration", [["Configuration", clean(electrical.grid).toLowerCase().includes("off") ? "Solar + battery" : "Hybrid — grid + solar + battery"], ["Customer objective", clean(electrical.systemGoal)], ["Design focus", essentialKw ? "Selected essential loads during outages" : "Load and backup confirmation required"], ["System status", "PRELIMINARY RECOMMENDATION"]], "red");
  y -= Math.max(pvHeight, configurationHeight) + 15;
  section("Design basis");
  const basis = [["Peak sun hours", "4.5 h/day"], ["PV performance factor", "78%"], ["Inverter headroom", "25%"], ["Backup requirement", backupHours ? `${formatNumber(backupHours, 1)} hours` : "To be confirmed"]];
  ...
  y -= 73;
  labeledText("Engineering explanation", "The recommended inverter size uses the recorded simultaneous peak demand together with an operating reserve for temporary load variation and appliance startup. The PV capacity estimate uses recorded daily energy, solar-resource assumptions and normal conversion losses.");
  section("Betech technical recommendation");
  const recommendation = reportRecommendationLabel(input.report);
  const recommendationText = input.report.recommendation.type === "CUSTOM_QUOTATION"
  ...
  page.drawText(`${formatNumber(inverterKw, 1)} kW Hybrid Inverter  ·  ${formatNumber(batteryKwh)} kWh Lithium Storage  ·  ${panelCount || "Indicative"} × 600W Solar Panels`, { x: MARGIN + 17, y: y - 49, font: bold, size: 8, color: rgb(1, 0.93, 0.93), maxWidth: A4[0] - MARGIN * 2 - 34 });
  ...
  y -= 108;
  if (input.report.recommendation.productUrl) labeledText("Selected Betech product", `${recommendation} has been selected from the live Betech website catalogue. View system or request quotation: ${input.report.recommendation.productUrl}`);
  ...
  const evidenceLabels = Object.keys(evidence).filter((key) => hasValue(evidence[key]));
  section("Site evidence and technical findings");
  if (evidenceLabels.length) {
    labeledText("Evidence status", `${evidenceLabels.length} site-evidence item(s) were recorded: ${evidenceLabels.slice(0, 8).join(", ")}. Evidence images and technical notes remain linked to the secure Betech site-visit workspace.`);
  } else {
  ...
  const findings = input.report.aiReview?.observations.length
    ...
    : [
        ...
        `An indicative ${formatNumber(pvKw)} kWp PV array supports the recorded energy profile subject to final roof and shading review.`,
      ];
  ...
  const signatureHeight = card(MARGIN, (A4[0] - MARGIN * 2 - 10) / 2, "Technician declaration", [["Status", input.report.signatures ? "DIGITALLY SIGNED" : "ASSESSMENT SUBMITTED"], ["Site assessor", input.report.signatures?.technicianName || input.report.submittedByName], ["Assessment", input.visitRef], ["Signed", issueDateTime]], "plain");
