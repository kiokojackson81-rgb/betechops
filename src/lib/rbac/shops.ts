...
export async function requireShopAccess(opts: { shopId: string; minRole?: 'ATTENDANT' | 'SUPERVISOR' }) {
  ...
  if (adminCheck.ok) return { ok: true as const, actor: (adminCheck as any).session };

  const session = (adminCheck as any).session;
  ...
  const assignment = await prisma.userShop.findUnique({ where: { userId_shopId: { userId: user.id, shopId: opts.shopId } } }).catch(() => null);
