...
const VALID_TRANSITIONS: Record<SiteVisitStatus, readonly SiteVisitStatus[]> = {
  PAYMENT_PENDING: ["PAYMENT_PENDING", "PAYMENT_FAILED", "PENDING", "CLOSED", "CANCELLED"],
  PAYMENT_FAILED: ["PAYMENT_FAILED", "PAYMENT_PENDING", "CLOSED", "CANCELLED"],
  PENDING: ["PENDING", "TECHNICIAN_ASSIGNED", "CLOSED", "CANCELLED"],
  TECHNICIAN_ASSIGNED: ["TECHNICIAN_ASSIGNED", "PENDING", "ASSESSED", "CLOSED", "CANCELLED"],
  ASSESSED: ["ASSESSED", "TECHNICIAN_ASSIGNED", "QUOTED", "CLOSED", "CANCELLED"],
  QUOTED: ["QUOTED", "ASSESSED", "CLOSED", "CANCELLED"],
  // Legacy values are retained so historic visits can still be corrected and
  // completed without a data migration.
  SCHEDULED: ["SCHEDULED", "PENDING", "TECHNICIAN_ASSIGNED", "VISITED", "ASSESSED", "CLOSED", "CANCELLED"],
  VISITED: ["VISITED", "SCHEDULED", "ASSESSED", "QUOTED", "CLOSED", "CANCELLED"],
  CLOSED: ["CLOSED"],
  CANCELLED: ["CANCELLED"],
};
