...
/** Server-controlled automatic issuance uses the configured supervisor signature. */
...
/** Both issuance paths validate evidence and freeze signature/stamp snapshots. */
export async function issueProfessionallyApprovedCertificate(input: {
  ...
  return tx.commissioningSession.update({
    ...
    data: {
      ...
      audit: appendCommissioningAudit(session.audit, { at: issuedAt.toISOString(), action: input.automaticSource ? "CERTIFICATE_AUTO_ISSUED_WITH_SUPERVISOR_SIGNATURE" : "CERTIFICATE_ISSUED_AFTER_PROFESSIONAL_CERTIFICATION", actorId: input.automaticSource ? input.approvedById || null : input.approvedById || session.technicianId, detail: { automaticSource: input.automaticSource || null, technicianId: session.technicianId, certificateNo, professional: input.professional.name, licenceNumber: input.professional.licenceNumber } }),
    },
  });
