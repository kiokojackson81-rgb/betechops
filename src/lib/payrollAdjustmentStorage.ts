...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE TYPE "PayrollAdjustmentType" AS ENUM ('CHAMA', 'LATENESS', 'DISCIPLINE', 'BONUS', 'COMMISSION_TOPUP', 'OTHER', 'CASH_ADVANCE');
  END IF;
END $$;
`,
    `
...
BEGIN
  IF EXISTS (
    ...
    WHERE lower(t.typname) = lower('payrolladjustmenttype')
      AND e.enumlabel = 'CASH_ADVANCE'
  ) THEN
