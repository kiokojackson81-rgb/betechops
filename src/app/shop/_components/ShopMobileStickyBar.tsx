...
export default function ShopMobileStickyBar({ productId, productName, price }: ShopMobileStickyBarProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-[#7a0000]/10 bg-white/95 px-3 pb-[calc(env(safe-area-inset-bottom)+0.8rem)] pt-3 shadow-[0_-18px_40px_rgba(15,23,42,0.12)] backdrop-blur lg:hidden">
      <div className="mx-auto grid max-w-[1380px] gap-2">
        <div className="flex items-center justify-between gap-3 rounded-[20px] border border-[#7a0000]/8 bg-[#fcfaf7] px-3.5 py-2.5">
          ...
          <Link
            href={quoteHref}
            className="inline-flex min-h-[2.6rem] shrink-0 items-center justify-center rounded-[16px] border border-[#7a0000]/18 bg-white px-3.5 py-2 text-[12px] font-bold text-slate-950 shadow-[0_10px_18px_rgba(15,23,42,0.05)]"
          >
            Quote
          </Link>
        </div>
        <div className="grid grid-cols-[1.15fr_0.95fr] gap-2">
          <AddToCartButton
            productId={productId}
