...
function getCategoryHref(slug: string) {
  return slug === "request-quote"
    ? SHOP_REQUEST_QUOTE_HREF
    : getShopCategoryHref(slug);
}
...
export default function ShopHero({
  categories,
  heroImageUrl = "/agents/hero-generated-v2.png",
}: ShopHeroProps) {
  const otherCategory = categories.find(
    (category) => category.slug === "other-categories",
  );
  const categoryList = [
    ...categories
      .filter((category) => category.slug !== "other-categories")
      .slice(0, 9),
    ...
    <section className="pt-2.5 sm:pt-4">
      <div className={shopStyles.shell}>
        <div className="grid gap-3 lg:grid-cols-[0.24fr_0.52fr_0.24fr]">
          <aside
            className={`${shopStyles.lightCard} relative z-40 hidden h-fit overflow-visible lg:block`}
          >
                ...
                <div key={category.slug} className="group relative">
                  <Link
                    ...
                    aria-haspopup={
                      ...
                      getShopCategoryDefinition(category.slug)?.subcategories
                        .length
                        ? "menu"
                        : undefined
                    }
                      ...
                      <div className="mt-2 grid gap-1.5">
                        ...
                        {(category.slug === "other-categories"
                          ? SHOP_CATEGORY_DEFINITIONS.filter(
                              (entry) =>
                                getShopCategoryDepartment(entry) === "GENERAL",
                            ).map((entry) => ({
                              value: entry.value,
                              label: entry.label,
                              isCategory: true,
                          ...
                          <Link
                            ...
                            href={
                              subcategory.isCategory
                                ? getShopCategoryHref(subcategory.value)
                                : getShopSubcategoryHref(
                                    category.slug,
                                    subcategory.value,
                                  )
          ...
          <div
            className={`${shopStyles.darkPanel} overflow-hidden p-3 sm:p-4 lg:p-5`}
          >
            <div className="grid gap-3 md:grid-cols-[1.08fr_0.92fr] md:items-center">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-[13px] leading-5 text-white/78 sm:text-sm sm:leading-6">
                  Genuine solar products with warranty support, Nairobi pickup
                  and countrywide delivery.
                </p>
                <div className="mt-4 grid gap-2.5 sm:grid-cols-2">
                  <Link
                    href={SHOP_ALL_PRODUCTS_HREF}
                    prefetch={false}
                    className={`${shopStyles.goldButton} min-h-[2.9rem] w-full`}
                  >
                    See All Products
                  ...
                  <Link
                    href={SHOP_REQUEST_QUOTE_HREF}
                    className={`${shopStyles.secondaryButton} min-h-[2.9rem] w-full bg-white/92`}
                  >
                    Request Free Quote
                ...
                <Link
                  ...
                  className="mt-2.5 inline-flex min-h-[2.55rem] items-center justify-center rounded-2xl border border-white/25 bg-white/8 px-4 text-sm font-bold text-white transition hover:border-[#f2b20f]/60 hover:bg-white/15"
                >
                  View all departments
                </Link>
                <Link
                ...
                <div className="mt-3 text-[12px] leading-5 text-white/72 sm:text-sm">
                  We deliver and install solar systems anywhere in Kenya, with
                  support for both small orders and complete system setups.
                </div>
              </div>
              ...
              <div className="overflow-hidden rounded-[22px] border border-white/10 bg-white/8 shadow-[0_18px_40px_rgba(0,0,0,0.18)] backdrop-blur sm:rounded-[24px]">
                <div className="relative h-44 sm:h-52 md:h-[18rem]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4">
                    ...
                    <div className="mt-2 max-w-sm text-base font-black leading-5 text-white sm:text-xl sm:leading-6">
                      Panels, batteries, inverters, pumps and full kits in one
                      place.
                    </div>
                  </div>
                </div>
