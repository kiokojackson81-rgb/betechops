...
export default function ShopLipaPolePoleStarter({
  ...
  return (
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[80] bg-slate-950/75 sm:flex sm:items-center sm:justify-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="lpp-dialog-title">
          <div className="flex h-[100dvh] w-full flex-col overflow-hidden bg-[#fcfaf7] sm:max-h-[92vh] sm:max-w-5xl sm:rounded-[30px] sm:border sm:border-white/20 sm:shadow-[0_35px_100px_rgba(15,23,42,0.45)]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
              <div className="grid min-h-full lg:grid-cols-[0.82fr_1.18fr]">
                <aside className="border-b border-[#7a0000]/10 bg-[radial-gradient(circle_at_top,rgba(242,178,15,0.20),transparent_42%),linear-gradient(160deg,#fffaf0_0%,#f7ede2_100%)] p-4 sm:p-6 lg:border-b-0 lg:border-r lg:p-8">
                  <div className="mx-auto max-w-sm lg:sticky lg:top-8">
                    ...
                    <h2 className="mt-4 text-xl font-black leading-tight text-slate-950 sm:text-2xl">{product.name}</h2>
                ...
                <section className="p-4 pb-[max(2rem,env(safe-area-inset-bottom))] sm:p-6 lg:p-8">
                  {step === "setup" ? (
                    <form onSubmit={startBooking} className="mx-auto max-w-2xl">
                      ...
                      <div className="mt-6 grid gap-4 sm:grid-cols-2">
                        ...
                        <label className="grid gap-2 text-sm font-bold text-slate-700 sm:col-span-2">Target completion date<input value={form.expectedCompletionDate} onChange={(event) => setForm((current) => ({ ...current, expectedCompletionDate: event.target.value }))} className={inputClass} type="date" required /></label>
                      </div>
