...
export default function ReceiptFormClient({ onCreated }: { onCreated?: (receipt: any) => void }) {
  ...
  return (
    <div className="mx-auto max-w-6xl p-6 space-y-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <select value={attendantId ?? ""} onChange={(e) => setAttendantId(e.target.value || null)} className="w-full rounded-lg border border-slate-300 bg-slate-50 text-slate-900 px-3 py-2 placeholder-slate-400">
          ...
          <div>
            ...
            <select value={docType} onChange={(e) => setDocType(e.target.value)} className="w-full rounded-lg border border-slate-300 bg-slate-50 text-slate-900 px-3 py-2 placeholder-slate-400">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" value={serial} onChange={(e) => setSerial(e.target.value)} placeholder="Serial" />
          </div>
          <div>
            <label className="text-sm text-slate-600">Customer Name</label>
            <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Customer name" />
          </div>
          <div>
            <label className="text-sm text-slate-600">Customer Phone</label>
            <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="07..." />
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="text-sm text-slate-600">Customer Email (for sending)</label>
            <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="email@example.com" />
          </div>
      ...
      <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="space-y-2">
          {items.map((it) => (
            <div key={it.id} className="grid gap-2 md:grid-cols-6 items-center border-b border-slate-200 pb-2">
              <input className="col-span-2 rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400 px-2 py-1" value={it.title} onChange={(e) => updateRow(it.id, { title: e.target.value })} placeholder="Item description" />
              <input type="number" min={1} className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 px-2 py-1" value={it.quantity} onChange={(e) => updateRow(it.id, { quantity: Math.max(1, Number(e.target.value || 1)) })} />
              <input type="number" min={0} className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 px-2 py-1" value={it.unitPrice as any} onChange={(e) => updateRow(it.id, { unitPrice: e.target.value === "" ? "" : Number(e.target.value) })} placeholder="Unit price" />
              {showSerials ? (
                <input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400 px-2 py-1" value={it.serial} onChange={(e) => updateRow(it.id, { serial: e.target.value })} placeholder="Serial / IMEI" />
              ) : (
                <div />
              )}
              {showWarranty ? (
                <select className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 px-2 py-1" value={it.warranty} onChange={(e) => updateRow(it.id, { warranty: e.target.value })}>
        ...
        <div className="mt-2">
          <Button variant="primary" onClick={addRow} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md">+ Add item</Button>
        </div>
      </section>

      <div className="grid gap-4 md:grid-cols-3">
        <div>
          <label className="text-sm text-slate-600">Tax % (optional)</label>
          <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" type="number" value={taxRate} onChange={(e) => setTaxRate(Number(e.target.value || 0))} />
          <label className="inline-flex items-center mt-1 text-sm text-slate-600"><input type="checkbox" checked={showTax} onChange={(e) => setShowTax(e.target.checked)} className="mr-2" /> Show tax on document</label>
        </div>
        <div>
          <label className="text-sm text-slate-600">Discount (KES)</label>
          <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" type="number" value={discount} onChange={(e) => setDiscount(Number(e.target.value || 0))} />
        ...
        <div>
          ...
          {docType === "LAYAWAY" && (
            <div className="mt-3 space-y-1">
              ...
              <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" type="number" value={deposit} onChange={(e) => setDeposit(Number(e.target.value || 0))} />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <Input className="rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400" value={warrantyText} onChange={(e) => setWarrantyText(e.target.value)} placeholder="Global warranty text (optional)" />
        </div>
        <div>
          <label className="text-sm text-slate-600">General notes / terms</label>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full rounded-lg border border-slate-300 bg-slate-50 text-slate-900 placeholder-slate-400 p-2 h-full min-h-[60px]" />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm card-top-accent">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleSave} disabled={saving} className="rounded-md bg-emerald-600 px-5 py-2 text-white hover:bg-emerald-700">{saving ? "Saving..." : "Save to System & Print"}</Button>
        </div>
      </div>
    </div>
  );
