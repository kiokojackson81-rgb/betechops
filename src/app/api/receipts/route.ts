  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const orderUpsert = await tx.order.upsert({
        ...
        create: {
          orderNumber: serial,
          customerName: payload?.customerName ?? payload?.customer ?? null,
          // optional fields
          attendantId: attendantId ?? null,
          shopId: shopId ?? '',
          status: docType === 'LAYAWAY' ? 'PENDING' : 'PAID',
          totalAmount: Number(total) || 0,
          paidAmount: docType === 'LAYAWAY' ? Number(payload?.deposit || 0) : Number(total) || 0,
        ...
        update: {
          ...
          paidAmount: docType === 'LAYAWAY' ? Number(payload?.deposit || 0) : Number(total) || undefined,
        },
      });
      ...
      for (const it of createdItems) {
        await tx.orderItem.create({ data: { orderId: orderUpsert.id, productId: it.product.id, quantity: it.qty, sellingPrice: Number(it.unitPrice || 0), serial: it.serial ?? null, warranty: it.warranty ?? null } });
      }
      ...
      const receiptData = {
        ...
        totals: { subtotal, tax: taxAmount, total, balance: docType === 'LAYAWAY' ? (total - Number(payload?.deposit || 0)) : 0 },
        data: payload,
      } as any;
      ...
      const provisional = await tx.commissionRecord.create({ data: { orderId: orderUpsert.id, attendantId: attendantId ?? null, amount: null, status: 'PENDING', data: { subtotal, tax: taxAmount, total } } });
      ...
      if (Number(total) >= IMMEDIATE_THRESHOLD && attendantId) {
        ...
        const totalsAgg = await tx.order.aggregate({ where: { attendantId, createdAt: { gte: period.startDate, lte: period.endDate }, status: 'PAID' }, _sum: { totalAmount: true, paidAmount: true } });
        ...
        const updatedCommission = await tx.commissionRecord.update({ where: { id: provisional.id }, data: { amount: String(salesCommission), status: 'RELEASED', releasedAt: new Date(), periodId: period.id } });
        // Upsert attendant balance to reflect immediate release
        try {
          if (attendantId) {
            await tx.balance.upsert({ where: { userId: attendantId }, create: { userId: attendantId, available: Number(salesCommission), pending: 0 }, update: { available: { increment: Number(salesCommission) } as any } as any });
          }
        } catch (e) {
          // best-effort: if balance model not present or operation fails, continue
          console.error('Failed to upsert balance on commission release', e);
        }
