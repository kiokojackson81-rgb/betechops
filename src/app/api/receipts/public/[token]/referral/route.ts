...
import { createReferralFromReview, createReferralSchema, ensureReviewInvitationForReceipt } from "@/lib/reviewsReferrals";
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const receipt = await prisma.receipt.findFirst({ where: { data: { path: ["publicReceiptToken"], equals: token } }, select: { id: true } });
