...
export async function POST(req: NextRequest, context: { params: { userId: string } } | { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    const actorId = (guard.session?.user as any)?.id ?? (await getActorId());
    await prisma.actionLog.create({ data: { actorId: actorId!, entity: 'Balance', entityId: userId, action: 'ADJUST', before: Prisma.JsonNull, after: bal } });
