  ...
  try {
    ...
    if (useOpenAI && client) {
      try {
        const messages = [
          {
            role: "system",
            content:
              "You are a helpful assistant that normalizes delivery addresses for Kenya. Return a single concise, human-readable normalized address. Fix common spacing/casing issues, expand obvious abbreviations (e.g., 'Rd' -> 'Road', 'St' -> 'Street') but do not invent missing city/county names. Output ONLY the normalized address text with no commentary.",
          },
        ...
        return NextResponse.json({ address: collapsed });
      } catch (aiErr) {
