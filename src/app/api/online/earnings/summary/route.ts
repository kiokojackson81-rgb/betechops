...
export async function GET(req: Request) {
  ...
  const usePosTotals = isJeniffer || (user as any)?.attendantCategory === "DIRECT_SALES_OPS";

  const now = new Date();
  const { tiers } = await getOrCreateCommissionPeriod(now);
  const period = getTradingPeriodFor(now);
  const jenifferPosSummary = usePosTotals ? await summarizePosReceiptsForPeriod({ ...period, userId }) : null;

  const [summary, marketingSummary, supportSummary, payrollRow] = await Promise.all([
  ...
  const baseResponse = {
    ...
    salesCommission: payrollRow.commissionDirect || payrollRow.commissionTotal || salesCommission,
    ...
    grossCommission: payrollRow.commissionGross || grossCommission,
    ...
    totalEarnings: payrollRow.totalEarnings || totalEarnings,
    totalDeductions: payrollRow.totalDeductions || totalDeductions,
    netPay: payrollRow.netPay || netPay,
