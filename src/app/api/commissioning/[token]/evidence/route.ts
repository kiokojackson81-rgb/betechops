...
// Request proxies reject oversized multipart bodies before this route can
// respond. The client prepares camera photos to stay safely below this limit.
...
export async function POST(req: Request, context: ParamsContext) {
  ...
  if (!["DRAFT", "RETURNED_FOR_CORRECTION"].includes(session.status))
    return NextResponse.json({ error: "Certificate Issued — View Only" }, { status: 409 });
  ...
  if (file.size > MAX_EVIDENCE_SIZE) return NextResponse.json({ error: "This photo is too large for a mobile upload. Retake it at a lower resolution or choose a photo smaller than 3 MB." }, { status: 400 });
