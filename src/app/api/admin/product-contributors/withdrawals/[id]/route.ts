...
import {
  ...
  requireProductContributorAdmin,
...
export async function PATCH(
  ...
  if (!parsed.success)
    return noStoreJson(
      { error: "Invalid withdrawal update." },
      { status: 400 },
    );
  ...
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(
    `UPDATE "ProductContributorWithdrawal"
     ...
     RETURNING "id", "status", "amountKes", "contributorId"`,
    parsed.data.status,
  ...
  if (!rows[0])
    return noStoreJson(
      { error: "Withdrawal was already processed or was not found." },
      { status: 409 },
    );
  const withdrawal = rows[0] as {
    id: string;
    status: "PAID" | "REJECTED";
    amountKes: number;
    contributorId: string;
  };
  void notifyProductContributorWithdrawal({
  ...
  return noStoreJson({ ok: true, withdrawal });
}
