...
export async function GET(request: NextRequest) {
  ...
  const [payments, successful, unmatched] = await Promise.all([
    ...
    // attempts have no transaction time, so keep them after settled payments
    // and order those pending records by their newest creation time.
    prisma.mpesaPayment.findMany({
      where,
      include,
      orderBy: [
        { transactionAt: { sort: "desc", nulls: "last" } },
        { createdAt: "desc" },
      ],
      take: 250,
    }),
