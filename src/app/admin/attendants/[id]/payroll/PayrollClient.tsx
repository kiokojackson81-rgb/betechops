...
export default function PayrollClient({
  ...
  async function fetchCashAdvances() {
    ...
    try {
      ...
      const res = await fetch(url, { credentials: "same-origin" });
      if (!res.ok) throw new Error("Failed to load cash advance requests");
        ...
        for (const row of rows) {
          if (!next[row.id]) {
            next[row.id] = {
              ...
              comment: String(row.adminComment ?? ""),
              saving: false,
            };
          }
        }
      ...
      showToast(err?.message || "Failed to load cash advance requests", "error");
    } finally {
  ...
  const decideCashAdvance = async (requestId: string, decision: "APPROVED" | "REJECTED") => {
    ...
    try {
      ...
      const payload = await res.json().catch(() => null);
      if (!res.ok) {
        throw new Error(payload?.error || "Failed to update cash advance request");
      }
      ...
      showToast(err?.message || "Failed to update cash advance request", "error");
    } finally {
      setAdvanceDrafts((current) => ({
        ...current,
        ...
        <div className="mt-3 space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="mt-2 space-y-3">
              {loadingCashAdvances ? <div className="text-xs text-slate-400">Loading cash advance requests...</div> : null}
              ...
              {cashAdvances.map((item) => {
                const draft = advanceDrafts[item.id] ?? {
                  ...
                  comment: String(item.adminComment ?? ""),
                  saving: false,
                };
                ...
                return (
                  <div key={item.id} className="rounded-xl bg-slate-950/60 px-4 py-4">
                    ...
                    {item.adminComment ? <p className="mt-2 text-xs text-slate-400">Comment: {item.adminComment}</p> : null}
                    {isPending ? (
