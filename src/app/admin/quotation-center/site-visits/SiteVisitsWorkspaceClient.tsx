...
export default function SiteVisitsWorkspaceClient({ staffOptions, initialQuoteRef, basePath = "/admin/quotation-center/site-visits" }: Props) {
  ...
  const fee = getStandardSiteVisitFee(form.county);
  useEffect(() => {
    if (fee != null && (!form.visitFee || Number(form.visitFee) === 2_000 || Number(form.visitFee) === 5_000)) {
