...
type Recipient = { id?: string; name: string; phone: string; message: string | null; ready?: boolean };
type SmsState = { recipients?: Recipient[]; technician: Recipient | null; customer: Recipient | null; error: string | null; history: Array<{ id: string; recipientName: string; phone: string; kind: string; status: string; error: string | null; createdAt: string; message: string }> };
...
export default function ProjectSmsPanel({ receiptId }: { receiptId: string }) {
  ...
  async function send(action: "technician" | "customer" | "prepare", recipientId?: string) {
    setBusy(true); setMessage("");
    try {
      const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, recipientId }) });
      const payload = await response.json();
      await load();
  ...
  return <section className="mt-4 space-y-3 rounded-xl border border-cyan-400/20 p-4 text-sm text-slate-200">
    ...
    {[...(data?.recipients || (data?.technician ? [data.technician] : [])).map(recipient => ({ kind: "technician" as const, recipient })), ...(data?.customer ? [{ kind: "customer" as const, recipient: data.customer }] : [])].map(({ kind, recipient }) => {
      if (!recipient) return null;
      return <article key={recipient.id || kind} className="space-y-3 rounded-xl bg-slate-950 p-4"><h3 className="font-bold">{kind === "technician" ? "Installer / agent commissioning link" : "Customer project documents"}</h3><p>To: {recipient.name}<br />Mobile: {recipient.phone || "No phone number saved"}</p>{recipient.message ? <p className="break-words rounded-lg border border-white/10 p-3 leading-6">{recipient.message}</p> : null}{kind === "customer" && !recipient.ready ? <button disabled={busy} type="button" onClick={() => void send("prepare")} className="rounded-lg bg-amber-300 px-3 py-2 font-bold text-slate-950">Prepare / retry customer documents</button> : <button type="button" disabled={busy || !recipient.phone || !recipient.message} onClick={() => void send(kind, recipient.id)} className="rounded-lg bg-cyan-300 px-3 py-2 font-bold text-slate-950 disabled:opacity-40">Send / Resend {kind === "technician" ? "Commissioning Link SMS" : "Customer Documents SMS"}</button>}</article>;
    })}
