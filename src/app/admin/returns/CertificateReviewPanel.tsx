...
export default function CertificateReviewPanel({ receiptId, showSms = true, onCertified }: { receiptId: string; showSms?: boolean; onCertified?: () => void }) {
  ...
  return <section className="mt-4 space-y-4 rounded-xl border border-cyan-400/20 p-4 text-sm text-slate-200">
    <button type="button" disabled={busy} onClick={() => void load()} className="rounded-lg bg-cyan-300 px-4 py-2 font-bold text-slate-950 disabled:opacity-50">{busy ? "Loading…" : "Certificates & warranty documents"}</button>
    {message ? <p role="status" className="rounded-lg bg-slate-950 p-3">{message}</p> : null}
    {review ? <>
      <h3 className="font-bold">Certificate status — {session.status === "AWAITING_PROFESSIONAL_REVIEW" ? "Ready for automatic certification" : String(session.status || "").replaceAll("_", " ")}</h3>
