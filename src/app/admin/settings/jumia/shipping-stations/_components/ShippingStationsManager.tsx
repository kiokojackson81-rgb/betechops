...
export default function ShippingStationsManager({ shops }: { shops: Shop[] }) {
  ...
  useEffect(() => {
    ...
    async function loadDefaults() {
      try {
        ...
        const d = j?.defaults || {};
        if (!cancelled) setProviderId(d?.[shopId]?.providerId || '');
      } catch {}
    }
    if (shopId) void loadDefaults();
  ...
  async function discoverProviders() {
    ...
    try {
      ...
      const first = items[0];
      if (!first?.id) { setProviders([]); return; }
  ...
  function applyProvider(p: any) {
    const id = p?.id || p?.providerId || '';
    if (id) setProviderId(String(id));
  }
      ...
      <div className="rounded-xl border border-white/10 bg-[var(--panel,#121723)] p-3">
        ...
        {providers.length > 0 ? (
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {providers.map((p) => (
              <div key={String(p?.id ?? p?.providerId)} className="p-3 rounded border border-white/10 bg-black/10">
          ...
          <div className="mt-3 text-sm opacity-70">No providers loaded. Enter an order number and click Discover.</div>
        )}
      </div>
    </div>
  );
