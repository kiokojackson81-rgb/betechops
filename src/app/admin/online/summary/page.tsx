...
export default async function AdminOnlineSummaryPage() {
  ...
  const [
    ...
    safe("manual weekly sales", () => [] as any[], () =>
      ...
      <section className="rounded-2xl border border-white/10 bg-slate-900/40 p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">
              Current trading period: {period.label}. Snapshot below uses the last 4 full Monday–Sunday weeks within this period.
            </p>
          </div>
          <Link
              ...
              <table className="w-full min-w-[760px] text-left text-sm">
                ...
                <tbody>
                  {manualAggRows.map((row) => (
                    <tr key={`${row.platform}-${row.shopId ?? "none"}-${row.attendantId ?? "none"}`} className="border-t border-white/5">
