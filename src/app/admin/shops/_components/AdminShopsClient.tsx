...
export default function AdminShopsClient({ initial }: { initial: ShopSummary[] }) {
  ...
  <ShopForm onCreatedAction={(j) => onShopCreated({ id: j.id, name: j.name, platform: j.platform ?? undefined })} />
        <div className="mt-4">
          <h3 className="font-semibold">Create Attendant</h3>
          <AttendantForm shops={shops.map(s => ({ id: s.id, name: s.name }))} onCreatedAction={(u, assigned) => onAttendantCreated(u, assigned)} />
        </div>
      </div>
      <div className="p-4 border rounded">
