...
function productSlug(value: string) { return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "product"; }
function productWebsiteUrl(product: Product) { return `${PUBLIC_SHOP_ORIGIN}${getShopProductHref(productSlug(product.name), product.id)}`; }
    ...
    <div className="mx-auto max-w-7xl space-y-7">
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 sm:p-7">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div><h2 className="text-2xl font-black">Your products</h2><p className="mt-1 text-sm text-slate-400">Published products can be opened directly on the Betech website.</p></div>
          <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-sm font-bold text-emerald-200">{products.length} product{products.length === 1 ? "" : "s"} created</div>
        </div>
        {!products.length ? <p className="mt-5 text-slate-400">Your products will appear here after you create them.</p> : <>
          <div className="mt-5 grid gap-3 md:hidden">
            {products.map((product) => {
              const published = product.showInShop && product.ecommerceVisible;
              const href = productWebsiteUrl(product);
              return <article key={product.id} className="rounded-2xl border border-white/10 bg-slate-950 p-4">
                <div className="flex gap-3">{product.mainImageUrl ? <img src={product.mainImageUrl} alt="" className="h-16 w-16 rounded-xl object-cover" /> : <div className="grid h-16 w-16 place-items-center rounded-xl bg-slate-800 text-xs font-black text-slate-400">{product.name.slice(0, 2).toUpperCase()}</div>}<div className="min-w-0 flex-1"><p className="text-xs text-slate-400">{product.sku}</p>{published ? <a href={href} target="_blank" rel="noreferrer" className="mt-1 block truncate font-black text-white hover:text-emerald-200 hover:underline">{product.name}</a> : <h3 className="mt-1 truncate font-black">{product.name}</h3>}<p className="mt-1 text-sm font-bold text-emerald-300">{money(product.sellingPrice)}</p></div></div>
                <div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-slate-800 px-2.5 py-1 text-slate-300">{SHOP_CATEGORY_OPTIONS.find((item) => item.value === product.category)?.label || product.category}</span><span className={`rounded-full px-2.5 py-1 font-bold ${published ? "bg-emerald-400/15 text-emerald-200" : "bg-amber-400/15 text-amber-100"}`}>{published ? "Live on website" : "Needs image to publish"}</span></div>
                <div className="mt-4 flex gap-2">{published ? <a href={href} target="_blank" rel="noreferrer" className="rounded-lg border border-emerald-400/40 px-3 py-2 text-sm font-bold text-emerald-200">Preview website</a> : null}<button onClick={() => { setEditingId(product.id); setForm(toForm(product)); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="rounded-lg border border-cyan-400/40 px-3 py-2 text-sm font-bold text-cyan-300">Edit product</button></div>
              </article>;
            })}
          </div>
          <div className="mt-5 hidden overflow-x-auto rounded-2xl border border-white/10 md:block"><table className="w-full min-w-[860px] text-sm"><thead className="border-b border-white/10 bg-slate-950/70 text-left text-xs uppercase tracking-wider text-slate-400"><tr><th className="px-4 py-3">Product</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Website</th><th className="px-4 py-3">Updated</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody>{products.map((product) => {
            const published = product.showInShop && product.ecommerceVisible;
            const href = productWebsiteUrl(product);
            const category = SHOP_CATEGORY_OPTIONS.find((item) => item.value === product.category)?.label || product.category;
            return <tr key={product.id} className="border-b border-white/10 last:border-0 hover:bg-white/[0.025]"><td className="px-4 py-4"><div className="flex items-center gap-3">{product.mainImageUrl ? <img src={product.mainImageUrl} alt="" className="h-14 w-14 rounded-xl object-cover" /> : <div className="grid h-14 w-14 place-items-center rounded-xl bg-slate-800 text-xs font-black text-slate-400">{product.name.slice(0, 2).toUpperCase()}</div>}<div className="min-w-0"><p className="font-mono text-[11px] text-slate-500">{product.sku}</p>{published ? <a href={href} target="_blank" rel="noreferrer" className="block max-w-[260px] truncate font-bold text-white hover:text-emerald-200 hover:underline">{product.name}</a> : <p className="max-w-[260px] truncate font-bold text-white">{product.name}</p>}<p className="mt-1 text-xs text-slate-400">{product.brand || "No brand"}{product.warrantyPeriod ? ` · ${product.warrantyPeriod}` : ""}</p></div></div></td><td className="px-4 py-4 text-slate-300"><p>{category}</p>{product.shopSubcategory ? <p className="mt-1 text-xs text-slate-500">{product.shopSubcategory}</p> : null}</td><td className="px-4 py-4 font-black text-emerald-300">{money(product.sellingPrice)}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${published ? "bg-emerald-400/15 text-emerald-200" : "bg-amber-400/15 text-amber-100"}`}>{published ? "Live on website" : "Needs image"}</span></td><td className="px-4 py-4 text-slate-400">{new Date(product.updatedAt).toLocaleDateString("en-KE")}</td><td className="px-4 py-4"><div className="flex justify-end gap-2">{published ? <a href={href} target="_blank" rel="noreferrer" className="rounded-lg border border-emerald-400/40 px-3 py-2 text-xs font-bold text-emerald-200">Preview</a> : null}<button onClick={() => { setEditingId(product.id); setForm(toForm(product)); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="rounded-lg border border-cyan-400/40 px-3 py-2 text-xs font-bold text-cyan-300">Edit</button></div></td></tr>;
          })}</tbody></table></div>
        </>}
      </section>
      </>}
