...
export default function ContributorDashboard() {
  ...
  return <main className="min-h-screen bg-[#07121a] px-4 py-8 text-slate-100 sm:px-8">
    <div className="mx-auto max-w-7xl space-y-7">
      ...
      {categoryOpen ? <div className="fixed inset-0 z-50 flex justify-end bg-black/60" role="dialog" aria-modal="true" aria-label="Select category"><section className="h-full w-full max-w-lg overflow-y-auto bg-slate-50 p-6 text-slate-900 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Categories</h2><button onClick={() => setCategoryOpen(false)} className="text-2xl text-slate-500">×</button></div><input autoFocus value={categoryQuery} onChange={(e) => setCategoryQuery(e.target.value)} className="mt-5 w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-amber-500" placeholder="Search for a category" />{recentCategories.length ? <div className="mt-6"><h3 className="font-black">Recently used categories</h3><div className="mt-3 space-y-2">{recentCategories.map((value) => { const category = SHOP_CATEGORY_OPTIONS.find((item) => item.value === value); return category ? <button key={value} onClick={() => chooseCategory(value)} className="block w-full rounded-xl border border-slate-200 px-4 py-3 text-left font-semibold hover:border-amber-500">{category.label}</button> : null; })}</div></div> : null}<div className="mt-6"><h3 className="font-black">All categories</h3><div className="mt-3 space-y-1">{filteredCategories.map((category) => <button key={category.value} onClick={() => chooseCategory(category.value)} className={`flex w-full items-center justify-between rounded-xl px-4 py-3 text-left hover:bg-amber-50 ${form.category === category.value ? "bg-amber-100 font-black" : ""}`}>{category.label}<span className="text-amber-600">›</span></button>)}{!filteredCategories.length ? <p className="px-4 py-3 text-sm text-slate-500">No matching categories.</p> : null}</div></div></section></div> : null}
    </div>
  </main>;
}
