  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-8">
      <div className="max-w-2xl text-center">
        <h1 className="text-2xl font-semibold">Marketing Tracker (temporary)</h1>
        <p className="mt-2 text-sm text-slate-400">Temporary placeholder page to unblock build. Restore full implementation from git history.</p>
      </div>
    </main>
  );
}
...
type MarketingDailyFormState = { date: string; dayOfWeek: DayName; fields: Record<string, boolean | number | string | null> };
type ReceiptItem = { id: string; productName: string; buyingPrice: number | "" };
type ReceiptRow = { id: string; receiptNumber: string; sellingTotal: number | ""; paymentMethod: "MPESA" | "CASH" | ""; items: ReceiptItem[] };
...
const defaultFormState = (): MarketingDailyFormState => {
  ...
  return { date: todayStr, dayOfWeek: day, fields: { ...dynamic } };
};
...
const newSaleRow = (): ReceiptRow => ({
  ...
  sellingTotal: "",
  paymentMethod: "",
  items: [{ id: typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : Math.random().toString(36).slice(2), productName: "", buyingPrice: "" }],
});

const pillClass = (checked: boolean) =>
  `rounded-full border px-4 py-2 text-sm font-medium transition ${checked ? "border-emerald-400 bg-emerald-400 text-black shadow-lg shadow-emerald-500/20" : "border-slate-700 bg-slate-800 text-slate-200 hover:border-slate-500"}`;
...
export default function MarketingTrackerPage() {
  ...
  const [weeklyVideoShootParticipated, setWeeklyVideoShootParticipated] = useState(false);
  const [weeklyVideoCount, setWeeklyVideoCount] = useState<number | "">("");
  const [periodSummary, setPeriodSummary] = useState<null | any>(null);

  const config = useMemo(() => marketingDayConfigs.find((c) => c.day === form.dayOfWeek) ?? marketingDayConfigs[0], [form.dayOfWeek]);

  useEffect(() => { setForm((prev) => ({ ...prev, dayOfWeek: deriveDayOfWeek(prev.date) })); }, [form.date]);
  useEffect(() => { if (!periodSummary) return; const timer = setTimeout(() => setPeriodSummary(null), 5 * 60 * 1000); return () => clearTimeout(timer); }, [periodSummary]);

  const groupedYesNo = useMemo(() => {
    const groups = new Map<string, typeof config.yesNoFields>();
    (config?.yesNoFields || []).forEach((f) => { if (!groups.has(f.section)) groups.set(f.section, []); groups.get(f.section)?.push(f); });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!res.ok) { router.replace("/attendant/login"); return; }
        const data = await res.json().catch(() => null);
        const user = data?.user; if (!user) { router.replace("/attendant/login"); return; }
        const role = user.role as string | undefined; const category = user.attendantCategory as string | undefined;
        if (role === "ADMIN") return; if (category !== "DIRECT_SALES_OPS") { const dest = getLandingPage(category ?? null, role); router.replace(dest); }
      } catch (err) { router.replace("/attendant/login"); }
    })();
  }, [router]);

  const updateField = (key: string, value: boolean | number | string | null) => setForm((prev) => ({ ...prev, fields: { ...prev.fields, [key]: value } }));

  const totals = useMemo(() => {
    const totalSales = receipts.reduce((sum, r) => sum + (typeof r.sellingTotal === "number" ? r.sellingTotal : Number(r.sellingTotal || 0)), 0);
    const totalProfit = receipts.reduce((sum, r) => sum + ((typeof r.sellingTotal === "number" ? r.sellingTotal : Number(r.sellingTotal || 0)) - r.items.reduce((s, it) => s + (typeof it.buyingPrice === "number" ? it.buyingPrice : Number(it.buyingPrice || 0)), 0)), 0);
    const totalItems = receipts.reduce((sum, r) => sum + r.items.length, 0);
    return { totalSales, totalProfit, totalItems };
  }, [receipts]);

  const commissionInfo = useMemo(() => calculateCumulativeCommission(totals.totalSales), [totals.totalSales]);
  const tradingPeriod = useMemo(() => getTradingPeriodFor(new Date(form.date)), [form.date]);
  const tradingPeriodLabel = tradingPeriod?.label;

  const updateReceipt = (id: string, patch: Partial<ReceiptRow>) => setReceipts((rows) => rows.map((row) => (row.id === id ? { ...row, ...patch } : row)));
  ...
  const addItem = (receiptId: string) => setReceipts((rows) => rows.map((r) => r.id === receiptId ? { ...r, items: [...r.items, { id: typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : Math.random().toString(36).slice(2), productName: "", buyingPrice: "" }] } : r));

  const updateItem = (receiptId: string, itemId: string, patch: Partial<ReceiptItem>) => setReceipts((rows) => rows.map((r) => r.id === receiptId ? { ...r, items: r.items.map((it) => (it.id === itemId ? { ...it, ...patch } : it)) } : r));

  const removeItem = (receiptId: string, itemId: string) => setReceipts((rows) => rows.map((r) => r.id === receiptId ? { ...r, items: r.items.filter((it) => it.id !== itemId).length > 0 ? r.items.filter((it) => it.id !== itemId) : r.items } : r));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    receipts.forEach((r, i) => {
      ...
      r.items.forEach((it, j) => { if (!it.productName || it.productName.trim() === "") errors.push(`Receipt ${i + 1}, item ${j + 1}: missing product name`); if (it.buyingPrice === "" || Number.isNaN(Number(it.buyingPrice))) errors.push(`Receipt ${i + 1}, item ${j + 1}: invalid buying price`); });
    });

    (config.textFields || []).forEach((f) => { const raw = form.fields[f.key]; if (!raw || String(raw).trim() === "") errors.push(`${f.key}: required`); });
    (config.numericFields || []).forEach((f) => { const raw = form.fields[f.key]; if (raw === "" || raw === null || raw === undefined || Number.isNaN(Number(raw))) errors.push(`${f.key}: required numeric`); });

    if (errors.length > 0) { showToast(errors.slice(0, 5).join("; "), "error"); setSubmitting(false); return; }
    ...
    try {
      ...
      Object.entries(marketingFieldTypes).forEach(([key, type]) => {
        ...
        else text[key] = String(raw || "");
      });

      const payload = { receipts, form, yesNo, numeric, text };
      const res = await fetch("/api/attendants/daily-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), credentials: "same-origin" });
      if (!res.ok) { const msg = await res.text().catch(() => "Failed to save report"); showToast(msg, "error"); setSubmitting(false); return; }
      showToast("Report saved", "success");
    } catch (err) { showToast("Failed to save report", "error"); } finally { setSubmitting(false); }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <form onSubmit={handleSubmit} className="mx-auto max-w-6xl p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <Input type="date" value={form.date} onChange={(e) => setForm((prev) => ({ ...prev, date: e.target.value }))} className="w-full rounded-xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-slate-100" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-xs uppercase tracking-wide text-slate-400">Day of week</label>
                <select value={form.dayOfWeek} onChange={(e) => setForm((prev) => ({ ...prev, dayOfWeek: e.target.value as DayName }))} className="w-full rounded-xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-slate-100">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12 items-start">
          <section className="lg:col-span-8 space-y-6">
            ...
            <Card className="border-slate-800 bg-slate-900/60 shadow-xl shadow-black/20">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <div className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-200">Auto-loaded from selected day</div>
              </div>

              <div className="space-y-6">
                {groupedYesNo.map(([section, fields]) => (
                  <div key={section} className="space-y-3">
                    <div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-slate-200">{section}</h3></div>
                    <div className="flex flex-wrap gap-2">{fields.map((f) => (<button type="button" key={f.key} onClick={() => updateField(f.key, !Boolean(form.fields[f.key]))} className={pillClass(Boolean(form.fields[f.key]))}>{f.label}</button>))}</div>
                  </div>
                ))}
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="w-full">
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            <button type="button" onClick={() => { setWeeklyMeetingAttended(true); updateField("weeklyMeetingAttended", true); }} className={pillClass(weeklyMeetingAttended)}>Attended weekly marketing meeting</button>
                            <button type="button" onClick={() => { setWeeklyMeetingAttended(false); updateField("weeklyMeetingAttended", false); }} className={pillClass(!weeklyMeetingAttended)}>Did not attend</button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-full">
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            <button type="button" onClick={() => { setWeeklyVideoShootParticipated(true); updateField("weeklyVideoShootParticipated", true); }} className={pillClass(weeklyVideoShootParticipated)}>Participated in weekly video shoot</button>
                            <button type="button" onClick={() => { setWeeklyVideoShootParticipated(false); updateField("weeklyVideoShootParticipated", false); }} className={pillClass(!weeklyVideoShootParticipated)}>Did not participate</button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-full">
                          ...
                          <div className="mt-2">
                            <Input type="number" min={0} value={String(weeklyVideoCount)} onChange={(e) => { const v = e.target.value === "" ? "" : Math.max(0, Number(e.target.value)); setWeeklyVideoCount(v === "" ? "" : Number(v)); updateField("weeklyVideoCount", v === "" ? "" : Number(v)); }} className="w-28 rounded-full border border-slate-800 bg-slate-950/80 px-3 py-2 text-center text-slate-100" />
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {(config.numericFields || []).length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-semibold text-slate-200">Numeric checks</h3>
                    <div className="grid gap-3 md:grid-cols-2">{(config.numericFields || []).map((f) => (<div key={f.key} className="space-y-2"><label className="text-xs uppercase tracking-wide text-slate-400">{f.label}</label><Input type="number" min={f.min} value={String(form.fields[f.key] ?? "")} onChange={(e) => updateField(f.key, e.target.value)} className="w-full rounded-xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-slate-100" /></div>))}</div>
                  </div>
                )}
                ...
                {(config.textFields || []).length > 0 && (
                  <div className="space-y-3">
                    ...
                    <div className="grid gap-3">{(config.textFields || []).map((f) => (<div key={f.key} className="space-y-2"><label className="text-xs uppercase tracking-wide text-slate-400">{f.label}</label><Textarea value={String(form.fields[f.key] ?? "")} onChange={(e) => updateField(f.key, e.target.value)} placeholder={f.placeholder} rows={3} className="w-full rounded-xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-slate-100" /></div>))}</div>
                  </div>
                )}
              </div>
            </Card>
          ...
          <div className="lg:col-span-4">
            <StatsCard periodLabel={periodSummary?.period.label ?? tradingPeriodLabel ?? "Nov 25, 2025 — Dec 24, 2025"} receipts={receipts.length} salesKes={totals.totalSales} items={totals.totalItems} commissionKes={commissionInfo.commission} nextTarget={commissionInfo.nextTarget ?? 0} />
          </div>
        </div>
      </form>
    </div>
...
export default function MarketingTrackerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/attendants/daily-report", {
        method: "POST",
      ...
      if (!res.ok) {
        const msg = await res.text().catch(() => "Failed to save report");
        showToast(msg, "error");
        setSubmitting(false);
        return;
      }
          ...
          <div className="mt-6">
            <Card className="border-emerald-700/60 bg-emerald-900/20 text-emerald-100 shadow-xl shadow-emerald-900/30">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-xs text-emerald-200">
                  This panel auto-hides after 5 minutes. Commission shown is cumulative
