...
export default async function CustomerProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const receipt = await prisma.receipt.findUnique({ where: { id }, select: { data: true, order: { select: { id: true } }, commissioningSession: { select: { status: true, certificateNo: true, customerAcknowledgedAt: true, customerTermsAcceptedAt: true } } } });
  ...
  return <div className="grid min-w-0 gap-4">
    ...
    {!isCancelled && receipt?.commissioningSession?.status === "ISSUED" ? <ProjectCompletionActions receiptId={id} acknowledged={Boolean(receipt.commissioningSession.customerAcknowledgedAt && receipt.commissioningSession.customerTermsAcceptedAt)} /> : null}
