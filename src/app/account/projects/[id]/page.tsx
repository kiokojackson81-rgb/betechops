...
export default async function CustomerProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="grid min-w-0 gap-4">
    <section className={`${shopStyles.darkPanel} p-5 sm:p-7`}>
      ...
      <div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-black sm:text-3xl">{order.orderRef}</h1><p className="mt-2 text-sm text-white/70">Track payment, technical review, scheduling and installation from one place.</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${isCancelled ? "bg-rose-500/20 text-rose-100" : "bg-white/10 text-white"}`}>{isCancelled ? "Cancelled" : stages[Math.min(progress, stages.length - 1)][1]}</span></div>
      {isCancelled ? <p className="mt-6 rounded-xl border border-rose-300/30 bg-rose-300/10 p-4 text-sm font-semibold text-rose-100">This installation project has been cancelled. Please contact Betech if you need assistance.</p> : <div className="mt-6 grid gap-2 sm:grid-cols-3 lg:grid-cols-7">{stages.map(([, label], index) => <div key={label} className={`rounded-xl border p-3 text-xs font-bold ${index <= progress ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-100" : "border-white/10 bg-white/5 text-white/50"}`}><div className="mb-2">{index <= progress ? <CheckCircle2 className="h-4 w-4" /> : <CircleDot className="h-4 w-4" />}</div>{label}</div>)}</div>}
    </section>
    <section className="grid gap-4 lg:grid-cols-2">
      <div className={`${shopStyles.lightCard} p-5`}><h2 className="font-black text-[#7a0000]">Payment Summary</h2><dl className="mt-4 grid gap-3 text-sm">{[["Project value", formatCurrency(flow.projectValue)], ["Payment terms", flow.paymentTerm.replaceAll("_", " ")], ["Installation fee", formatCurrency(flow.depositRequiredAmount || flow.projectValue)], ["Paid", formatCurrency(flow.totalPaidAmount)], ["Balance", formatCurrency(flow.remainingAmount)], ["Status", flow.paymentStatus.replaceAll("_", " ")], ["Method", mpesaPayment ? "M-Pesa" : "—"], ["Receipt", mpesaPayment?.receiptNumber || "—"], ["Paid at", mpesaPayment?.transactionAt ? formatDate(mpesaPayment.transactionAt.toISOString()) : "—"], ["Payer", maskedPayer]].map(([key, value]) => <div key={key} className="flex justify-between gap-4"><dt className="text-slate-500">{key}</dt><dd className="text-right font-bold">{value}</dd></div>)}</dl></div>
