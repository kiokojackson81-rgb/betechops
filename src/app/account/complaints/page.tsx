...
export default async function CustomerComplaintsPage() {
  ...
  return <section className={`${shopStyles.lightCard} w-full min-w-0 p-5 sm:p-7`}>
    <div className="flex flex-wrap items-start justify-between gap-4"><div><div className={shopStyles.sectionEyebrow}>My complaints / support cases</div><h1 className="mt-3 text-2xl font-black sm:text-3xl">Track support from report to resolution</h1><p className="mt-2 text-sm text-slate-600">Review status changes, support replies, evidence, and the complete handling timeline.</p></div><Link href="/support/report-issue" className={shopStyles.primaryButton}>Report a new issue</Link></div>
    ...
    </Link>) : <div className="rounded-[22px] border border-dashed border-[#7a0000]/15 p-10 text-center xl:col-span-2"><MessageSquareWarning className="mx-auto h-9 w-9 text-[#7a0000]" /><div className="mt-3 font-black">No complaints or support cases yet</div><p className="mt-2 text-sm text-slate-500">Use Report a new issue when you need help connected to your account.</p></div>}</div>
  </section>;
}
