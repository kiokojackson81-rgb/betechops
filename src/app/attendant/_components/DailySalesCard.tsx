...
export default function DailySalesCard() {
  ...
  async function submit() {
    ...
    if (!Number.isFinite(value) || value <= 0) {
      toast("Enter a valid sales amount", "error");
      return;
    }
    setBusy(true);
    try {
      ...
      toast("Daily sales submitted", "success");
      setAmount("");
      setNotes("");
      await fetchHistory();
    } catch (err) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[linear-gradient(135deg,rgba(20,30,45,.9),rgba(20,30,45,.7))] p-4 shadow-lg shadow-black/30">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
        <div>
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white outline-none focus:border-emerald-400/60"
          />
        </div>
        <button
          onClick={submit}
          disabled={busy}
          className="self-end rounded-lg bg-emerald-500/80 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-500 focus:outline-none disabled:opacity-60"
        >
          {busy ? "Saving..." : "Submit"}
        </button>
      </div>
      ...
      <div className="mt-3">
        ...
        <textarea
          ...
          maxLength={200}
          rows={2}
          placeholder="Optional notes (cash vs MPESA, special orders, etc.)"
          className="mt-1 w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white outline-none focus:border-emerald-300/60"
        />
      </div>
