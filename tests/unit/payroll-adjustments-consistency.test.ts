...
describe("payroll adjustment consistency", () => {
  it("includes Jeniffer's KES 22,000 addition in admin, earnings and payslip totals", () => {
    ...
    const totalEarnings = 25_000 + 182_494 + adjustment.totalBonus;
    const row: PayrollRow = {
      ...
      commission: 182_494, commissionGross: 182_494, commissionDirect: 182_494,
      commissionMarketplaceJumia: 0, commissionMarketplaceKilimall: 0, commissionTotal: 182_494,
      commissionBreakdown: {}, bonusTotal: adjustment.totalBonus, deductionTotal: 0,
    ...
    expect(row.totalEarnings).toBe(229_494);
    expect(row.netPay).toBe(229_494);

    const card = buildEarningsCardBreakdown(row);
    expect(card.totalEarnings).toBe(229_494);
    expect(card.netPay).toBe(229_494);
    ...
    expect(payslip.totalEarnings).toBe(229_494);
    expect(payslip.netPay).toBe(229_494);
    ...
    expect(pdfHtml).toContain("Ksh 22,000");
  });
});
